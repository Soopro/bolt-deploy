...
if(toggleTextWrapper('big')){clearEmptyElements(selectedNode,['big','small']);getCurrentSelection();return historyManager.record();}};insertMark=function(e){if(!inEditor(selectedNode,true)){return;}
if(toggleTextWrapper('mark')){clearEmptyElements(selectedNode,['mark']);getCurrentSelection();return historyManager.record();}};insertCode=function(e){var element;if(!inEditor(selectedNode)){return;}
element=findElement(selectedNode,'CODE');if(!element){if(!testBlackList(selectedNode)){selectedNode.outerHTML='<code>'+selectedNode.innerHTML+'</code>';selObj.extend(selectedNode,0);}}else{element.outerHTML='<div>'+element.innerHTML+'</div>';selObj.extend(element,0);}
return historyManager.record();};clearStyles=function(e){var content,element,i,l,len1,sel_elements;if(!inEditor(selectedNode,true)){return;}
sel_elements=getAllElementsFromSelection();for(l=0,len1=sel_elements.length;l<len1;l++){element=sel_elements[l];element=findStylesParent(element);element.removeAttribute('style');if(!element.innerHTML||!inEditor(element)){continue;}
content=element.innerHTML;i=0;while(i<styleTags.length){content=removeTag(content,styleTags[i],true);i+=1;}
element.innerHTML=removeInlineStyle(content);}
historyManager.record();return getCurrentSelection();};blowupHtml=function(e){return wildClear();};insertHR=function(e){return execCmd('insertHorizontalRule');};insertA=function(e){var link;if(inEditor(selectedNode,true)){if(typeof insertLinkHook==='function'){savedRange=saveSelection();insertLinkHook(savedRange.endOffset>0);}else{link=prompt('Link:','#');execCmd('createLink',false,link);}
...
return historyManager.record();}};toggleHtml=function(e){var content;if(!inEditor(selectedNode,true)){return;}
historyManager.record();if(typeof openHTMLHook==='function'){savedRange=saveSelection();openHTMLHook();}else{if($editor.tagName!=='textarea'){content=$editor.innerHTML;$editor.outerHTML='<textarea>'+content+'</textarea>';richEditorModel=false;}else{content=$editor.value;$editor.outerHTML='<div>'+content+'</div>';richEditorModel=true;}
if($toolbar){if(richEditorModel){$toolbar.removeAttribute('disabled');}else{$toolbar.setAttribute('disabled',true);}}}
return historyManager.record();};getCurrentSelection=function(){var node;node=void 0;selObj=window.getSelection();if(selObj.type!=='None'&&selObj.rangeCount>0){selRange=selObj.getRangeAt(0);startEl=selRange.startContainer;endEl=selRange.endContainer;node=selObj.anchorNode;selectedOrigin=node;while(node.nodeType!==1){node=node.parentNode;}
...
if(element.parentElement){elementTagName=element.parentElement.tagName;if(ref=elementTagName.toLowerCase(),indexOf.call(styleTags,ref)>=0){element=findStylesParent(element.parentElement);}else{element=element.parentElement;}}
...
return null;};clearEmptyElements=function(element,tags){var el,empty_els,l,len1,results,tag;if(!element||!inEditor(element,true)){return;}
element=element===$editor?element:element.parentElement;results=[];for(l=0,len1=tags.length;l<len1;l++){tag=tags[l];empty_els=element.querySelectorAll(tag);results.push((function(){var len2,m,results1;results1=[];for(m=0,len2=empty_els.length;m<len2;m++){el=empty_els[m];if(!el.innerHTML){results1.push(el.parentElement.removeChild(el));}else{results1.push(void 0);}}
return results1;})());}
...
return historyManager.record();};getAllElementsFromSelection=function(){var allSelected,allWithinRange,el,i,l,len1,sel_cac;sel_cac=selRange.commonAncestorContainer;if(sel_cac.nodeType===1){allWithinRange=sel_cac.getElementsByTagName("*");}else{allWithinRange=[sel_cac.parentNode];}
allSelected=[];i=0;el=void 0;for(l=0,len1=allWithinRange.length;l<len1;l++){el=allWithinRange[l];if(selObj.containsNode(el,true)&&el.nodeType===1){allSelected.push(el);}
i+=1;}
...
return console.info('---------------------------');};isJsonString=function(str){var e;try{JSON.parse(str);}catch(error){e=error;return false;}
return true;};createToolbarBtns=function(toolbar){var attr_tbs,btn,group,i,ico,j,k,tbs,val;attr_tbs=toolbar.getAttribute('toolbar-btns');if(!attr_tbs){return toolbar;}
tbs=attr_tbs.replace(/'/g,'"');if(isJsonString(tbs)){tbs=JSON.parse(tbs);}
if(typeof tbs!=='object'||!tbs.length||tbs.length===0){tbs=defaultToolbarBtns;}
i=0;while(i<tbs.length){group=document.createElement('div');j=0;while(j<tbs[i].length){val=tbs[i][j];if(toolbar.querySelector('.btn-'+val)){j+=1;continue;}
btn=document.createElement('button');btn.className='btn btn-'+val;ico=void 0;for(k in btnList){if(btnList[k].name===val){ico=btnList[k].ico;break;}}
if(ico){btn.innerHTML='<i class="ico-'+ico+'"></i>';}else{btn.innerHTML=val.toUpperCase();}
group.appendChild(btn);j+=1;}
if(group.childNodes.length>1){group.className='btn-group';toolbar.appendChild(group);}else{group=null;}
i+=1;}
return toolbar;};dispatchSAWEvent=function(){var event;if(!$editor){return;}
...
if(typeof toolbar==='string'){$toolbar=document.querySelector('[id='+toolbar+']');}else if(isHTMLElement(toolbar)){$toolbar=toolbar;}
...
style_el=document.createElement('style');style_el.setAttribute('saw-style','');style_el.innerHTML=['sup-angular-wysiwyg [preset],','[sup-angular-wysiwyg] [preset] ','{cursor: pointer;}','sup-angular-wysiwyg [preset] [prevent-touch],','[sup-angular-wysiwyg] [preset] [prevent-touch] ','{pointer-events: none;}'].join(' ');document.head.appendChild(style_el);$editor.setAttribute('contenteditable','true');if($toolbar){$toolbar=createToolbarBtns($toolbar);}
$property.order=parseInt($editor.getAttribute('order'))||0;$property.id=$editor.getAttribute('id')||Date.now();$property.name=$editor.getAttribute('name')||now;$editor.addEventListener('mousedown',focusHandler);$editor.addEventListener('click',selectHandler);$editor.addEventListener('dblclick',doubleClickHandler);if($toolbar){$toolbar.addEventListener('mousedown',focusHandler);}
$editor.addEventListener('keyup',selectHandler);window.addEventListener('mousedown',blurHandler);set_toolbar();setKeyboard($editor);historyManager.record();console.info('supAngularWysiwyg('+ver+'): '+$property.name+' inited');};destroy_editor=function(){$editor.removeEventListener('mousedown',focusHandler);$editor.removeEventListener('click',selectHandler);$editor.removeEventListener('dbclick',selectHandler);if($toolbar){$toolbar.removeEventListener('mousedown',focusHandler);}
$editor.removeEventListener('keyup',selectHandler);window.removeEventListener('mousedown',blurHandler);unset_toolbar();console.info('supAngularWysiwyg: destroyed');};set_toolbar=function(){var i,obj,results;if(!$toolbar){return;}
i=0;results=[];while(i<btnList.length){obj=$toolbar.querySelector('.btn-'+btnList[i].name);if(obj){if(btnList[i].name==='indent'){$btn_indent=obj;}
if(btnList[i].name==='outdent'){$btn_outdent=obj;}
evtManager.register(obj,eval(btnList[i].handler));}
results.push(i+=1);}
return results;};unset_toolbar=function(){var i,obj,results;if(!$toolbar){return;}
i=0;results=[];while(i<btnList.length){obj=$toolbar.querySelector('.btn-'+btnList[i].name);if(obj){evtManager.remove(obj,eval(btnList[i].handler));}
results.push(i+=1);}
...
presets=[];if(angular.isArray(scope.presets)){ref=scope.presets;for(l=0,len1=ref.length;l<len1;l++){p=ref[l];presets.push({name:p.name,type:p.type,figure:p.figure,code:p.code});}}
...
return obj;}; angular.clean_tags=function(str){if(!angular.isString(str)){str='';}
return str.replace(/<[^>]+>/gm,'');};angular.clean_space=function(content,force){if(!force){return content.replace(/\s{2,}/g," ").trim();}else{return content.replace(" ","").trim();}};angular.title=function(text){return text.charAt(0).toUpperCase()+text.slice(1).toLowerCase();};angular.lower=function(text){return text.toLowerCase();};angular.upper=function(text){return text.toUpperCase();};angular.clean_ng_attr=function(content){return content.replace(/ng-[a-zA-Z0-9_\-]+=["\'].*?["\']/ig,'');};angular.isDict=function(obj){return Boolean(angular.isObject(obj)&&!angular.isArray(obj));};angular.isUrl=function(url){var regex;if(typeof url!=='string'){return false;}
...
regex=/^[\w]+:/i;return Boolean(str.match(regex));};angular.addURLParams=function(url,input_params,concat,unique){var _key_sets,_params,add_params,i,idx,k,key,l,len1,len2,len3,m,n,p,params,url_hash,v;if(concat===void 0){concat=true;}
if(unique===void 0){unique=true;}
if(angular.isObject(input_params)&&!angular.isArray(input_params)){add_params=[];for(k in input_params){v=input_params[k];if(angular.isArray(v)){for(l=0,len1=v.length;l<len1;l++){i=v[l];add_params.push([k,i]);}}else{add_params.push([k,v]);}}}else if(angular.isString(input_params)){add_params=[[input_params,input_params]];}else{return'';}
params=_url_parse_qsl(url);url_hash=url.substring(url.indexOf('#'),url.length)||'';url=url.split('?')[0].split('#')[0];if(concat){params=params.concat(add_params);}else{params=add_params;}
if(unique){_key_sets={};_params=[];for(idx=m=0,len2=params.length;m<len2;idx=++m){p=params[idx];key=p[0];if(key in _key_sets){_params[_key_sets[key]]=p;}else{_params.push(p);_key_sets[key]=idx;}}
params=_params;}
for(n=0,len3=params.length;n<len3;n++){p=params[n];url=_add_url_param(url,p[0],p[1]);}
return url+url_hash;};angular.getURLParams=function(url,unique){var k,l,len1,param,params,params_qsl,v;params_qsl=_url_parse_qsl(url);params={};for(l=0,len1=params_qsl.length;l<len1;l++){param=params_qsl[l];k=param[0];v=param[1];if(params[k]&&!unique){if(!angular.isArray(params[k])){params[k]=[params[k]];}
params[k].push(v);}else{params[k]=v;}}
return params;};_add_url_param=function(url,key,value){var _add,item,l,len1;_add=function(url,key,value){var joint;joint=url.indexOf('?')>-1?'&':'?';key=encodeURIComponent(key);value=encodeURIComponent(value)||key;url=url+joint+key+'='+value;return url;};if(angular.isArray(value)){for(l=0,len1=value.length;l<len1;l++){item=value[l];url=_add(url,key,item);}}else{url=_add(url,key,value);}
return url;};_url_parse_qsl=function(url){var hash_idx,k,l,len1,pair,params_qsl,query_idx,query_vars,v;if(typeof url!=='string'){return{};}
hash_idx=url.indexOf('#');if(hash_idx>-1){url=url.substring(0,hash_idx);}
params_qsl=[];query_idx=url.indexOf('?');if(query_idx>-1){query_vars=url.split('?')[1].split('&');for(l=0,len1=query_vars.length;l<len1;l++){v=query_vars[l];pair=v.split('=');k=pair[0];v=pair[1]||null;if(!k){continue;}
params_qsl.push([k,v]);}}
return params_qsl;};angular.getPureURL=function(url){try{return url.split(/[?#]/)[0];}catch(error1){return'';}}; angular.dataURLToBlob=function(dataURL){var BASE64_MARKER,contentType,i,parts,raw,rawLength,uInt8Array;BASE64_MARKER=';base64,';if(dataURL.indexOf(BASE64_MARKER)===-1){parts=dataURL.split(',');contentType=parts[0].split(':')[1];raw=parts[1];return new Blob([raw],{type:contentType});}
