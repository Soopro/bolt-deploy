...
if(typeof selTextHook==='function'){return selTextHook(text);}};targetImg=function(img){var imgA,imgObj,new_src,src;if(typeof editMediaHook==='function'){$currentImage.element=img;imgObj={type:'image',src:img.getAttribute('src'),title:img.getAttribute('alt'),alt:img.getAttribute('alt'),class:img.getAttribute('class')};imgA=img.parentElement;if(imgA&&inEditor(imgA)&&imgA.tagName==='A'){$currentImage.link=imgA;imgObj.link=imgA.getAttribute('href');imgObj.target=imgA.getAttribute('target');imgObj.class=imgA.getAttribute('class');}else{$currentImage.link=null;}
...
if(typeof target==='object'){if(typeof handler==='function'){target.removeEventListener(event,handler);target.removeEventListener(event,baseEventHanlder);}}}};resetCurrentMedia=function(){$currentImage={link:void 0,element:void 0};$currentVideo={element:void 0};return $currentAudio={element:void 0};};resetCurrentLink=function(){return $currentLink={element:void 0};};doInsertImg=function(imgObj){var alt,class_,class_str,href,html,src,target,target_str;src=assertParam(imgObj.src);alt=assertParam(imgObj.title);href=assertParam(imgObj.href);target=assertParam(imgObj.target);class_=assertParam(imgObj.class);if(class_){class_str='class="'+class_+'"';}else{class_str='';}
if(href){if(target){target_str='target="'+target+'"';}else{target_str='';}
html='<a '+class_str+' '+target_str+' href="'+href+'">'+'<img src="'+src+'" alt="'+alt+'"></a>';}else{html='<img '+class_str+' src="'+src+'" alt="'+alt+'" />';}
return html;};doReplaceImg=function(imgObj){var alt,class_,class_str,href,img,src,target,target_str;img=$currentImage.element;if(!img||!imgObj){return;}
src=assertParam(imgObj.src);alt=assertParam(imgObj.title);href=assertParam(imgObj.href);target=assertParam(imgObj.target);class_=assertParam(imgObj.class);if(src){img.setAttribute('src',src);img.removeAttribute('ng-src');}
...
return new_list;};angular.capitalize=function(str){return str.charAt(0).toUpperCase()+str.slice(1);};angular.list2dict=function(list,key,key2){var dict,idx,item,l,len;if(typeof key!=='string'){key=null;}
...
try{orglist=str.split(',');}catch(error1){e=error1;orglist=[];}
