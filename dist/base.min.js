...
if(typeof hooks.selList==='function'){return hooks.selList(list);}};targetImg=function(img){var imgA,imgObj,new_src,src;if(typeof hooks.editMedia==='function'){$currentImage.element=img;imgObj={type:'image',src:img.getAttribute('src'),title:img.getAttribute('alt'),width:img.getAttribute('width')||img.style.width,height:img.getAttribute('height')||img.style.height,alt:img.getAttribute('alt'),class:img.getAttribute('class')};imgA=img.parentElement;if(imgA&&inEditor(imgA)&&imgA.tagName==='A'){$currentImage.link=imgA;imgObj.link=imgA.getAttribute('href');imgObj.target=imgA.getAttribute('target');imgObj.class=imgA.getAttribute('class');}else{$currentImage.link=null;}
...
if(typeof target==='object'){if(typeof handler==='function'){target.removeEventListener(event,handler);target.removeEventListener(event,baseEventHandler);}}}};resetCurrentMedia=function(){$currentImage={link:void 0,element:void 0};$currentVideo={element:void 0};return $currentAudio={element:void 0};};resetCurrentLink=function(){return $currentLink={element:void 0};};resetCurrentPreset=function(){return $currentPreset={element:void 0};};_pre_media_size=function(size_attr){if(!size_attr){size_attr='auto';}
if(!isNaN(size_attr)){size_attr=size_attr+'px';}
return size_attr;};doInsertImg=function(imgObj){var _height,_width,alt,class_,class_str,height,href,html,md_str,src,style_str,target,target_str,width;src=imgObj.src;alt=imgObj.title;href=imgObj.href;target=imgObj.target;width=imgObj.width;height=imgObj.height;class_=imgObj.class;if(class_){class_str='class="'+class_+'"';}else{class_str='';}
if(width||height){_width=_pre_media_size(width);_height=_pre_media_size(height);style_str='style="width:'+_width+';height:'+_height+';"';}else{style_str='';}
if(href){if(target){target_str='target="'+target+'"';}else{target_str='';}
md_str='media-src="'+src+'"';html='<a '+md_str+' '+class_str+' '+target_str+' href="'+href+'">'+'<img src="'+src+'" '+style_str+' alt="'+alt+'"></a>';}else{html='<img '+class_str+' '+style_str+' src="'+src+'" alt="'+alt+'" />';}
return html;};doReplaceImg=function(imgObj){var _h,_w,alt,class_,class_str,height,href,img,src,style_str,target,target_str,width;img=$currentImage.element;if(!img||!inEditor(img)||!imgObj){return;}
...
if(width||height){_w=_pre_media_size(width);_h=_pre_media_size(height);style_str='style="width:'+_w+';height:'+_h+';"';img.setAttribute('style','width:'+_w+';height:'+_h+';');}else{style_str='';img.removeAttribute('style');}
...
img.outerHTML='<a '+class_str+' '+target_str+' href="'+href+'">'+'<img src="'+src+'" '+style_str+' alt="'+alt+'"></a>';}}else{if($currentImage.link){$currentImage.link.outerHTML=$currentImage.link.innerHTML;}
if(class_){img.setAttribute('class',class_);}else{img.removeAttribute('class');}}
return img;};doInsertVideo=function(videoObj){var _height,_width,autoplay,class_,class_str,controls,height,html,looping,poster,src,style_str,width;src=videoObj.src;poster=videoObj.poster;width=videoObj.width;height=videoObj.height;controls=videoObj.controls?'controls':'';looping=videoObj.loop?'loop':'';autoplay=videoObj.autoplay?'autoplay':'';class_=videoObj.class;if(class_){class_str='class="'+class_+'"';}else{class_str='';}
if(width||height){_width=_pre_media_size(width);_height=_pre_media_size(height);style_str='style="width:'+_width+';height:'+_height+';"';}else{style_str='';}
html='<video '+looping+' '+autoplay+' '+controls+' '+class_str+' '+'src="'+src+'" '+style_str+' poster="'+poster+'" preload="none"></video>';return html;};doReplaceVideo=function(videoObj){var _h,_w,autoplay,class_,controls,looping,poster,src,video;video=$currentVideo.element;if(!video||!inEditor(video)||!videoObj){return;}
...
if(width||height){_w=_pre_media_size(width);_h=_pre_media_size(height);video.setAttribute('style','width:'+_w+';height:'+_h+';');}
