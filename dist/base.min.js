...
return historyManager.record();}};targetPreset=function(div){var presetObj,preset_code,preset_type;if(typeof hooks.editPreset==='function'){preset_type=div.getAttribute('preset');if(preset_type){try{preset_code=decodeURIComponent(div.getAttribute('code')||'');}catch(error){preset_code='';}
presetObj={type:preset_type,code:preset_code,rendered:div.innerHTML};$currentPreset.element=div;return hooks.editPreset(presetObj);}else{return resetCurrentPreset();}}};insertOrderedList=function(e){var content,node,ol;if(!inEditor(selectedNode,true)){return;}
...
createTextWrapper('mark',true);clearEmptyElements(selectedNode,['mark']);getCurrentSelection();return historyManager.record();};makeCode=function(e){var element;if(!inEditor(selectedNode)){return;}
...
selObj.extend(element,0);return historyManager.record();};clearFormat=function(e){var content,el,element,fmt_el,fmt_elements,i,j,k,len1,len2,sel_elements;if(!inEditor(selectedNode,true)){return;}
...
if(fmt_elements.length===1){clearTextWrapper(STYLE_TAGS);}else{for(k=0,len2=fmt_elements.length;k<len2;k++){element=fmt_elements[k];if(!element.innerHTML||!inEditor(element)){continue;}
...
return historyManager.record();};insertHR=function(e){return execCmd('insertHorizontalRule');};insertA=function(e){var link;if(inEditor(selectedNode,true)){if(typeof hooks.insertLink==='function'){savedRange=saveSelection();return hooks.insertLink(savedRange.endOffset>0);}else{link=prompt('Link:','#');execCmd('createLink',false,link);return historyManager.record();}}};insertMedia=function(e){var src;if(inEditor(selectedNode,true)){if(typeof hooks.insertMedia==='function'){savedRange=saveSelection();return hooks.insertMedia();}else{src=prompt('Img:','http://');if(src!==''&&src!=='http://'){execCmd('insertImage',false,src);}
return historyManager.record();}}};insertPreset=function(e){if(inEditor(selectedNode,true)){if(typeof hooks.insertPreset==='function'){savedRange=saveSelection();return hooks.insertPreset();}}};insertScript=function(e){var script;if(inEditor(selectedNode,true)){if(typeof hooks.insertScript==='function'){savedRange=saveSelection();return hooks.insertScript();}else{savedRange=saveSelection();script=prompt('Script:','');if(script!==''){execInsertScript(script);}
...
return results1;})());}
return results;};createTextWrapper=function(tag,toggle_mode){var content,el;if(!selectedNode){return;}
tag=tag.toLowerCase();el=document.createElement(tag);el.appendChild(selRange.cloneContents());if(toggle_mode&&el.querySelector(tag)){content=removeTag(el.innerHTML,tag);}else{content=el.outerHTML;}
...
window.removeEventListener('mousedown',blurHandler);unset_toolbar();unset_keyboard();console.info('supAngularWysiwyg: destroyed');};set_toolbar=function(){var btn,btn_el,j,len1,results;if(!$toolbar){return;}
results=[];for(j=0,len1=BTN_LIST.length;j<len1;j++){btn=BTN_LIST[j];btn_el=$toolbar.querySelector('.btn-'+btn.name);if(btn_el){evtManager.register(btn_el,eval(btn.handler));results.push($btn_elements_stack[btn.name]=btn_el);}else{results.push(void 0);}}
...
loaded={label:attrs.label||attrs.name,model:attrs.ngModel};$rootScope.$emit('saw.loaded',loaded,editor);if(root!==$rootScope){return root.$emit('saw.loaded',loaded,editor);}},0);editor.setHook('selText',function(el){return root.$emit('saw.select_text',editor,el);});editor.setHook('selList',function(el){return root.$emit('saw.select_list',editor,el);});editor.setHook('insertLink',function(has_selection){return root.$emit('saw.insert_link',editor,has_selection);});editor.setHook('editLink',function(link){return root.$emit('saw.edit_link',editor,link);});editor.setHook('insertMedia',function(){return root.$emit('saw.insert_media',editor);});editor.setHook('editMedia',function(media){return root.$emit('saw.edit_media',editor,media);});editor.setHook('insertPreset',function(){return root.$emit('saw.insert_preset',editor,presets);});editor.setHook('editPreset',function(curr_preset){return root.$emit('saw.edit_preset',editor,presets,curr_preset);});editor.setHook('insertScript',function(){return root.$emit('saw.insert_script',editor);});editor.setHook('focus',function(){return root.$emit('saw.focus',editor);});editor.setHook('blur',function(){return root.$emit('saw.blur',editor);});editor.setHook('openHTML',function(){return root.$emit('saw.open_html',editor,ngModel.$viewValue);});dirty=false;if(scope.default){default_data=scope.default;}else{default_data=$interpolate(element.html())($rootScope);}
