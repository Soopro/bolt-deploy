...
function minErr(module,ErrorConstructor){ErrorConstructor=ErrorConstructor||Error;var url='https://errors.angularjs.org/1.7.0/';var regex=url.replace('.','\\.')+'[\\s\\S]*';var errRegExp=new RegExp(regex,'g');return function(){var code=arguments[0],template=arguments[1],message='['+(module?module+':':'')+code+'] ',templateArgs=sliceArgs(arguments,2).map(function(arg){return toDebugString(arg,minErrConfig.objectMaxDepth);}),paramPrefix,i;message+=template.replace(/\{\d+\}/g,function(match){var index=+match.slice(1,-1);if(index<templateArgs.length){return templateArgs[index].replace(errRegExp,'');}
return match;});message+='\n'+url+(module?module+'/':'')+code;for(i=0,paramPrefix='?';i<templateArgs.length;i++,paramPrefix='&'){message+=paramPrefix+'p'+i+'='+encodeURIComponent(templateArgs[i]);}
...
if(optional&&!attrs[attrName])break;parentGet=$parse(attrs[attrName]);var isLiteral=parentGet.literal;var initialValue=destination[scopeName]=parentGet(scope);initialChanges[scopeName]=new SimpleChange(_UNINITIALIZED_VALUE,destination[scopeName]);removeWatch=scope.$watch(parentGet,function parentValueWatchAction(newValue,oldValue){if(oldValue===newValue){if(oldValue===initialValue||(isLiteral&&equals(oldValue,initialValue))){return;}
...
html.push('href="',url.replace(/"/g,'&quot;'),'">');addText(text);html.push('</a>');}};}]);})(window,window.angular);(function(window,angular){'use strict';angular.module('ngCookies',['ng']).info({angularVersion:'1.7.0'}).provider('$cookies',[function $CookiesProvider(){var defaults=this.defaults={};function calcOptions(options){return options?angular.extend({},defaults,options):defaults;}
...
var str=encodeURIComponent(name)+'='+encodeURIComponent(value);str+=path?';path='+path:'';str+=options.domain?';domain='+options.domain:'';str+=expires?';expires='+expires.toUTCString():'';str+=options.secure?';secure':'';var cookieLength=str.length+1;if(cookieLength>4096){$log.warn('Cookie \''+name+
'\' possibly not set or overflowed because it was too large ('+
cookieLength+' > 4096 bytes)!');}
return str;}
return function(name,value,options){rawDocument.cookie=buildCookieString(name,value,options);};}
$$CookieWriter.$inject=['$document','$log','$browser'];angular.module('ngCookies').provider('$$cookieWriter',function $$CookieWriterProvider(){this.$get=$$CookieWriter;});})(window,window.angular);(function(){var supWysiwygEditor,indexOf=[].indexOf;supWysiwygEditor=function(){var $btn_elements_stack,$currentAudio,$currentImage,$currentLink,$currentPreset,$currentVideo,$editor,$history,$history_step,$keymap,$options,$property,$toolbar,BTN_LIST,CELL_TAGS,STYLE_TAGS,baseEventHandler,blowupHtml,blurHandler,childOfElement,cleanStyles,clearEmptyElements,clearFormat,clearTextWrapper,createTextWrapper,destroySpan,destroy_editor,dispatchSAWEvent,doInsertAudio,doInsertImg,doInsertVideo,doReplaceAudio,doReplaceImg,doReplaceVideo,doubleClickHandler,endEl,errlog,evtManager,execCmd,execFormatBlock,execInsertLink,execInsertMedia,execInsertPreset,execInsertScript,execRemoveLink,execRemoveMedia,execRemovePreset,execReplaceLink,execReplaceMedia,execReplacePreset,expose,findAllTopElements,findCellEl,findElement,findSibling,findStylesParent,findTopElement,focusHandler,getAllElementsFromSelection,getCurrentSelection,getElementStyle,getSameParent,getSelectionHtml,historyManager,hooks,inEditor,indentText,init_editor,insertA,insertHR,insertMedia,insertOrderedList,insertPreset,insertScript,insertUnorderedList,isFamily,isHTMLElement,justifyCenter,justifyFull,justifyLeft,justifyRight,keyboardHandler,load_options,log,makeBold,makeCode,makeDiv,makeFontBig,makeFontNormal,makeFontSmall,makeH1,makeH2,makeH3,makeH4,makeH5,makeH6,makeItalic,makeMark,makeP,makePre,makeQuote,makeStrike,makeStyle,makeSub,makeSup,makeUnderline,now,outdentText,pasteHandler,pasteHtmlAtCaret,placeCaretAfterNode,reDo,removeInlineStyle,removeTag,resetCurrentLink,resetCurrentMedia,resetCurrentPreset,restoreSelection,richEditorModel,runtimeHandler,saveSelection,savedRange,selObj,selRange,selectedNode,selectedOrigin,setRange,setRangeByStartEnd,set_hook,set_keyboard,set_toolbar,smartFormatBlock,startEl,stripLayer,targetA,targetAudio,targetImg,targetListEl,targetPreset,targetText,targetVideo,toggleHtml,unDo,unset_keyboard,unset_toolbar,ver;ver='1.9.1';now=Date.now();$options={history:100};$property={id:'',order:0};$editor=void 0;$toolbar=void 0;selObj=void 0;selRange=void 0;selectedNode=void 0;selectedOrigin=void 0;startEl=void 0;endEl=void 0;savedRange=void 0;richEditorModel=true;hooks={focus:void 0,blur:void 0,selText:void 0,selList:void 0,insertLink:void 0,editLink:void 0,insertMedia:void 0,editMedia:void 0,insertPreset:void 0,editPreset:void 0,insertScript:void 0,openHTML:void 0};$currentImage={element:void 0,link:void 0};$currentVideo={element:void 0};$currentAudio={element:void 0};$currentLink={element:void 0};$currentPreset={element:void 0};$history=[];$history_step=null;$keymap={};$btn_elements_stack=[];BTN_LIST=[{name:'h1',handler:'makeH1'},{name:'h2',handler:'makeH2'},{name:'h3',handler:'makeH3'},{name:'h4',handler:'makeH4'},{name:'h5',handler:'makeH5'},{name:'h6',handler:'makeH6'},{name:'div',handler:'makeDiv'},{name:'p',handler:'makeP'},{name:'blockquote',handler:'makeQuote'},{name:'pre',handler:'makePre'},{name:'code',handler:'makeCode'},{name:'mark',handler:'makeMark'},{name:'style',handler:'makeStyle'},{name:'i',handler:'makeItalic'},{name:'b',handler:'makeBold'},{name:'strike',handler:'makeStrike'},{name:'underline',handler:'makeUnderline'},{name:'normal',handler:'makeFontNormal'},{name:'small',handler:'makeFontSmall'},{name:'big',handler:'makeFontBig'},{name:'sub',handler:'makeSub'},{name:'sup',handler:'makeSup'},{name:'ol',handler:'insertOrderedList'},{name:'ul',handler:'insertUnorderedList'},{name:'outdent',handler:'outdentText'},{name:'indent',handler:'indentText'},{name:'hr',handler:'insertHR'},{name:'img',handler:'insertMedia'},{name:'preset',handler:'insertPreset'},{name:'a',handler:'insertA'},{name:'jleft',handler:'justifyLeft'},{name:'jcenter',handler:'justifyCenter'},{name:'jright',handler:'justifyRight'},{name:'jfull',handler:'justifyFull'},{name:'redo',handler:'reDo'},{name:'undo',handler:'unDo'},{name:'clear',handler:'clearFormat'},{name:'cleanup',handler:'cleanStyles'},{name:'strip',handler:'stripLayer'},{name:'script',handler:'insertScript'},{name:'html',handler:'toggleHtml'}];CELL_TAGS=['ol','ul','li','td','th','tr','tbody','table','thead','tfoot','dl','dd','dt'];STYLE_TAGS=['b','i','u','s','mark','strong','sup','sub','small','big','strike','del','font','span'];runtimeHandler=function(e){var target;getCurrentSelection();target=findStylesParent(e.target||selectedNode);targetText(target);return targetListEl(target);};doubleClickHandler=function(e){var child_img,preset_target,tagname,target;getCurrentSelection();if(selectedNode&&selectedNode.tagName==='A'){target=selectedNode;}else{target=findStylesParent(e.target||selectedNode);}
preset_target=findElement(target,'DIV','preset');if(preset_target){target=preset_target;}
tagname=target.hasAttribute('preset')?'preset':target.tagName.toLowerCase();switch(tagname){case'preset':return targetPreset(target);case'img':return targetImg(target);case'video':return targetVideo(target);case'audio':return targetAudio(target);case'a':child_img=target.querySelector('img');if(child_img){return targetImg(child_img);}else{return targetA(target);}}};keyboardHandler=function(e){var allowed_keycodes,ref;allowed_keycodes=[8,46,17,91,88,16,13];if(ref=e.keyCode,indexOf.call(allowed_keycodes,ref)<0){return;}
if(e.type==='keydown'){switch(e.keyCode){case 16:$keymap['shift']=true;break;case 13:$keymap['enter']=true;}
if($keymap['shift']&&$keymap['enter']){e.preventDefault();return pasteHtmlAtCaret('<br>');}}else if(e.type==='keyup'){switch(e.keyCode){case 16:$keymap['shift']=false;break;case 13:$keymap['enter']=false;}
return historyManager.record();}};focusHandler=function(e){getCurrentSelection();savedRange=saveSelection();if(!childOfElement(e.target,$toolbar,true)){if(typeof hooks.focus==='function'){hooks.focus();}}};blurHandler=function(e){if(inEditor(e.target,true)){return;}
savedRange=saveSelection();if(!childOfElement(e.target,$toolbar,true)){$keymap={};if(typeof hooks.blur==='function'){hooks.blur();}}};pasteHandler=function(e){var text;if(e.clipboardData&&e.clipboardData.getData){e.preventDefault();text=e.clipboardData.getData("text/plain");return document.execCommand("insertText",false,text);}else if(window.clipboardData&&window.clipboardData.getData){e.preventDefault();text=window.clipboardData.getData("Text");return document.execCommand("insertText",false,text);}};targetText=function(text){var ref,text_types;text_types=['h1','h2','h3','h4','h5','h6','p','blockquote','pre','code','div'];if(!text||!inEditor(text)){text=selectedNode;}
if(!text||(ref=text.tagName.toLowerCase(),indexOf.call(text_types,ref)<0)){text=null;}
if(typeof hooks.selText==='function'){return hooks.selText(text);}};targetListEl=function(list){if(!list||list.tagName!=='LI'){list=null;}
if(typeof hooks.selList==='function'){return hooks.selList(list);}};targetImg=function(img){var imgA,imgObj,new_src,src;if(typeof hooks.editMedia==='function'){$currentImage.element=img;imgObj={type:'image',src:img.getAttribute('src'),title:img.getAttribute('alt'),width:img.getAttribute('width'),height:img.getAttribute('height'),alt:img.getAttribute('alt'),class:img.getAttribute('class')};imgA=img.parentElement;if(imgA&&inEditor(imgA)&&imgA.tagName==='A'){$currentImage.link=imgA;imgObj.link=imgA.getAttribute('href');imgObj.target=imgA.getAttribute('target');imgObj.class=imgA.getAttribute('class');}else{$currentImage.link=null;}
return hooks.editMedia(imgObj);}else{src=img.getAttribute('src');new_src=prompt('Img:',src);if(new_src){img.setAttribute('src',new_src);}else if(new_src!==null){img.parentElement.removeChild(img);}
return historyManager.record();}};targetVideo=function(video){var new_src,src,videoObj,video_source;if(typeof hookseditMedia==='function'){$currentVideo.element=video;try{video_source=video.querySelector('source').getAttribute('src');}catch(error){video_source='';}
videoObj={type:'video',src:video.getAttribute('src')||video_source,width:video.getAttribute('width'),height:video.getAttribute('height'),poster:video.getAttribute('poster'),controls:video.hasAttribute('controls'),loop:video.hasAttribute('loop'),autoplay:video.hasAttribute('autoplay'),class:video.getAttribute('class')};return hooks.editMedia(videoObj);}else{src=video.getAttribute('src');new_src=prompt('Video:',src);if(new_src){video.setAttribute('src',new_src);video.setAttribute('controls',true);}else if(new_src!==null){video.parentElement.removeChild(video);}
return historyManager.record();}};targetAudio=function(audio){var audioObj,new_src,src;if(typeof hooks.editMedia==='function'){$currentAudio.element=audio;audioObj={type:'audio',src:audio.getAttribute('src'),loop:audio.hasAttribute('loop'),autoplay:audio.hasAttribute('autoplay'),controls:audio.hasAttribute('controls'),class:audio.getAttribute('class')};return hooks.editMedia(audioObj);}else{src=audio.getAttribute('src');new_src=prompt('Audio:',src);if(new_src){audio.setAttribute('src',new_src);audio.setAttribute('controls',true);}else if(new_src!==null){audio.parentElement.removeChild(audio);}
return historyManager.record();}};targetA=function(a){var linkObj,new_url;if(typeof hooks.editLink==='function'){$currentLink.element=a;linkObj={href:a.getAttribute('href'),target:a.getAttribute('target'),class:a.getAttribute('class')};return hooks.editLink(linkObj);}else{new_url=prompt('Link:',url);if(new_url){a.setAttribute('href',new_url);}else if(new_url!==null){a.outerHTML=a.innerHTML;}
return historyManager.record();}};targetPreset=function(div){var presetObj,preset_type;if(typeof hooks.editPreset==='function'){preset_type=div.getAttribute('preset');if(preset_type){presetObj={type:preset_type,controls:true,rendered:div.innerHTML};$currentPreset.element=div;return hooks.editPreset(presetObj);}else{return resetCurrentPreset();}}};insertOrderedList=function(e){var content,node,ol;if(!inEditor(selectedNode,true)){return;}
if(inEditor(selectedNode)){execCmd('insertorderedlist');getCurrentSelection();ol=selectedNode.parentElement.parentElement;destroySpan(ol,true);if(ol.parentElement&&ol.parentElement.tagName==='P'){ol.parentElement.outerHTML=ol.parentElement.innerHTML;}}else{content=selectedNode.innerHTML;selectedNode.innerHTML='<ol><li>'+content+'</li></ol>';node=selectedNode.querySelector('li');setRange(node);}
historyManager.record();return getCurrentSelection();};insertUnorderedList=function(e){var content,node,ul;if(!inEditor(selectedNode,true)){return;}
if(inEditor(selectedNode)){execCmd('insertunorderedlist');getCurrentSelection();ul=selectedNode.parentElement.parentElement;destroySpan(ul,true);if(ul.parentElement&&ul.parentElement.tagName==='P'){ul.parentElement.outerHTML=ul.parentElement.innerHTML;}}else{content=selectedNode.innerHTML;selectedNode.innerHTML='<ul><li>'+content+'</li></ul>';node=selectedNode.querySelector('li');setRange(node);}
historyManager.record();return getCurrentSelection();};outdentText=function(e){if(findElement(selectedNode,'LI')){execCmd('outdent');historyManager.record();return getCurrentSelection();}};indentText=function(e){if(findElement(selectedNode,'LI')){execCmd('indent');historyManager.record();return getCurrentSelection();}};makeDiv=function(e){smartFormatBlock('div');return targetText();};makeH1=function(e){smartFormatBlock('h1');return targetText();};makeH2=function(e){smartFormatBlock('h2');return targetText();};makeH3=function(e){smartFormatBlock('h3');return targetText();};makeH4=function(e){smartFormatBlock('h4');return targetText();};makeH5=function(e){smartFormatBlock('h5');return targetText();};makeH6=function(e){smartFormatBlock('h6');return targetText();};makePre=function(e){smartFormatBlock('pre');return targetText();};makeQuote=function(e){smartFormatBlock('blockquote');return targetText();};makeP=function(e){smartFormatBlock('p');return targetText();};makeBold=function(e){return execCmd('bold');};makeItalic=function(e){return execCmd('italic');};justifyLeft=function(e){return execCmd('justifyLeft');};justifyRight=function(e){return execCmd('justifyRight');};justifyCenter=function(e){return execCmd('justifyCenter');};justifyFull=function(e){return execCmd('justifyFull');};makeStrike=function(e){return execCmd('strikeThrough');};makeUnderline=function(e){return execCmd('underline');};makeSup=function(e){return execCmd('superscript');};makeSub=function(e){return execCmd('subscript');};reDo=function(e){return historyManager.redo();};unDo=function(e){return historyManager.undo();};makeFontNormal=function(e){var els;if(!inEditor(selectedNode,true)){return;}
els=['big','small','sup','sub'];clearTextWrapper(els);getCurrentSelection();return historyManager.record();};makeFontSmall=function(e){if(!inEditor(selectedNode,true)){return;}
createTextWrapper('small');clearEmptyElements(selectedNode,['big','small']);getCurrentSelection();return historyManager.record();};makeFontBig=function(e){if(!inEditor(selectedNode,true)){return;}
createTextWrapper('big');clearEmptyElements(selectedNode,['big','small']);getCurrentSelection();return historyManager.record();};makeMark=function(e){if(!inEditor(selectedNode,true)){return;}
createTextWrapper('mark',1);clearEmptyElements(selectedNode,['mark']);getCurrentSelection();return historyManager.record();};makeStyle=function(e){var el,style_cls;if(!inEditor(selectedNode,true)){return;}
el=e.currentTarget||e.target;style_cls=el.getAttribute('text-style');if(!style_cls){return;}
createTextWrapper('span',2,{'class':style_cls});clearEmptyElements(selectedNode,['span']);getCurrentSelection();return historyManager.record();};makeCode=function(e){var element;if(!inEditor(selectedNode)){return;}
element=findElement(selectedNode,'CODE');if(element){element.outerHTML='<div>'+element.innerHTML+'</div>';}else{element=findCellEl(selectedNode);element.outerHTML='<code>'+element.innerHTML+'</code>';}
selObj.extend(element,0);return historyManager.record();};clearFormat=function(e){var content,el,element,fmt_el,fmt_elements,i,j,l,len1,len2,nosel,sel_elements;if(!inEditor(selectedNode,true)){return;}
sel_elements=getAllElementsFromSelection();fmt_elements=[];for(j=0,len1=sel_elements.length;j<len1;j++){el=sel_elements[j];fmt_el=findStylesParent(el);if(indexOf.call(fmt_elements,fmt_el)<0){fmt_elements.push(fmt_el);}}
nosel=selRange.startOffset===selRange.endOffset;if(!fmt_elements.length){return; }else if(fmt_elements.length===1&&!nosel){clearTextWrapper(STYLE_TAGS);}else{for(l=0,len2=fmt_elements.length;l<len2;l++){element=fmt_elements[l];if(!element.innerHTML||!inEditor(element)){continue;}
element.removeAttribute('style');content=element.innerHTML;i=0;while(i<STYLE_TAGS.length){content=removeTag(content,STYLE_TAGS[i]);i+=1;}
element.innerHTML=removeInlineStyle(content);}}
historyManager.record();return getCurrentSelection();};cleanStyles=function(e){var el,els,j,l,len1,len2,span,span_els;if(!$editor.innerHTML){return;}
els=$editor.querySelectorAll('[style]');for(j=0,len1=els.length;j<len1;j++){el=els[j];el.removeAttribute('style');}
span_els=$editor.querySelectorAll('span');for(l=0,len2=span_els.length;l<len2;l++){span=span_els[l];if(span.hasAttribute('class')){continue;}
span.outerHTML=span.innerHTML;}
historyManager.record();return getCurrentSelection();};stripLayer=function(e){var element;element=findTopElement(selectedNode);if(element){$editor.removeChild(element);}
historyManager.record();return dispatchSAWEvent();};blowupHtml=function(e){var current,end,i,resultObj,room,start,tmp_trigger,trigger;if(inEditor(selectedNode)){start=startEl.parentElement;end=endEl.parentElement;if(start!==end){resultObj=getSameParent(start,end);room=resultObj.point.childNodes;i=0;while(i<room.length){if(room[i].nodeType===1){tmp_trigger=false;if(room[i]===resultObj.start||room[i]===resultObj.end){if(trigger){tmp_trigger=false;}else{trigger=true;tmp_trigger=trigger;}}
if(trigger){room[i].outerHTML='<div>'+room[i].innerText+'</dvi>';}
trigger=tmp_trigger;}
i+=1;}}else{current=findCellEl(selectedNode);if(current){current=current.outerHTML='<div>'+current.innerText+'</div>';}else{selectedNode.outerHTML='<div>'+selectedNode.innerText+'</div>';}}}else if(inEditor(selectedNode,true)){$editor.innerHTML=$editor.innerText;}
return historyManager.record();};insertHR=function(e){return execCmd('insertHorizontalRule');};insertA=function(e){var link;if(inEditor(selectedNode,true)){savedRange=saveSelection();if(typeof hooks.insertLink==='function'){return hooks.insertLink(savedRange.endOffset!==savedRange.startOffset);}else{link=prompt('Link:','#');execCmd('createLink',false,link);return historyManager.record();}}};insertMedia=function(e){var src;if(inEditor(selectedNode,true)){savedRange=saveSelection();if(typeof hooks.insertMedia==='function'){return hooks.insertMedia();}else{src=prompt('Img:','http://');if(src!==''&&src!=='http://'){execCmd('insertImage',false,src);}
return historyManager.record();}}};insertPreset=function(e){if(inEditor(selectedNode,true)){savedRange=saveSelection();if(typeof hooks.insertPreset==='function'){return hooks.insertPreset();}}};insertScript=function(e){var script;if(inEditor(selectedNode,true)){savedRange=saveSelection();if(typeof hooks.insertScript==='function'){return hooks.insertScript();}else{script=prompt('Script:','');if(script!==''){execInsertScript(script);}
return historyManager.record();}}};toggleHtml=function(e){if(inEditor(selectedNode,true)&&typeof hooks.openHTML==='function'){savedRange=saveSelection();return hooks.openHTML();}};getCurrentSelection=function(){var node;node=void 0;selObj=window.getSelection();if(selObj.type!=='None'&&selObj.rangeCount>0){selRange=selObj.getRangeAt(0);startEl=selRange.startContainer;endEl=selRange.endContainer;node=selObj.anchorNode;selectedOrigin=node;while(node.nodeType!==1){node=node.parentNode;}
return selectedNode=node;}};setRange=function(element){var range,sel;range=document.createRange();range.selectNode(element);sel=window.getSelection();sel.removeAllRanges();return sel.addRange(range);};setRangeByStartEnd=function(element){var range,sel;range=document.createRange();range.setStartBefore(element);range.setEndAfter(element);sel=window.getSelection();sel.removeAllRanges();return sel.addRange(range);};inEditor=function(element,allowself){return childOfElement(element,$editor,allowself);};childOfElement=function(element,parent_element,allowself){var find,id,id_attr,tag;if(!element||!parent_element){return false;}else if(!element.tagName||!parent_element.tagName){return false;}else if(element===parent_element&&allowself){return true;}else{id_attr='saw-child-id';id='saw-'+Date.now().toString();element.setAttribute('saw-child-id',id);tag=element.tagName.toLowerCase();find=parent_element.querySelector(tag+'['+id_attr+'='+id+']');element.removeAttribute('saw-child-id');}
return Boolean(find);};saveSelection=function(){var sel;if(window.getSelection){sel=window.getSelection();if(sel.getRangeAt&&sel.rangeCount){return sel.getRangeAt(0);}}else if(document.selection&&document.selection.createRange){return document.selection.createRange();}
return null;};restoreSelection=function(range){var sel;if(!range){return;}
if(window.getSelection){sel=window.getSelection();sel.removeAllRanges();return sel.addRange(range);}else if(document.selection&&range.select){return range.select();}};isHTMLElement=function(o){var is_obj,is_obj_type,result;if(!o){return false;}
is_obj=o&&typeof o==='object'&&o!==null;is_obj_type=o.nodeType===1&&typeof o.nodeName==='string';result=is_obj&&is_obj_type;return result;};isFamily=function(elm1,elm2){var parent1,parent2;parent1=elm1.parentElement;parent2=elm2.parentElement;return elm1===elm2||parent1===elm2||parent2===elm1;};findStylesParent=function(element){var elementTagName,ref;elementTagName=element.tagName.toLowerCase();if(element.nodeType===1&&indexOf.call(STYLE_TAGS,elementTagName)<0){return element;}
if(inEditor(element.parentElement)){elementTagName=element.parentElement.tagName;if(ref=elementTagName.toLowerCase(),indexOf.call(STYLE_TAGS,ref)>=0){element=findStylesParent(element.parentElement);}else{element=element.parentElement;}}
return element;};findElement=function(element,tag,attr){if(!element||!inEditor(element)||!tag){return null;}
tag=tag.toUpperCase();if(element.tagName===tag){if(attr&&element.hasAttribute(attr)){return element;}else if(!attr){return element;}}
return findElement(element.parentElement,tag,attr);};findTopElement=function(element){if(!element){return element;}else if(element.parentElement===$editor&&element.nodeType===1){return element;}else if(inEditor(element.parentElement)){return findTopElement(element.parentElement);}
return null;};findAllTopElements=function(){var el;return(function(){var j,len1,ref,results;ref=$editor.childNodes;results=[];for(j=0,len1=ref.length;j<len1;j++){el=ref[j];if(el.nodeType===1){results.push(el);}}
return results;})();};clearEmptyElements=function(element,tags){var el,empty_els,j,len1,results,tag;if(!element||!inEditor(element,true)){return;}
element=element===$editor?element:element.parentElement;results=[];for(j=0,len1=tags.length;j<len1;j++){tag=tags[j];empty_els=element.querySelectorAll(tag);results.push((function(){var l,len2,results1;results1=[];for(l=0,len2=empty_els.length;l<len2;l++){el=empty_els[l];if(!el.innerHTML){results1.push(el.parentElement.removeChild(el));}else{results1.push(void 0);}}
return results1;})());}
return results;};createTextWrapper=function(tag,mode,attrs){var content,el,k,v;if(!selectedNode){return;}
if(!attrs||typeof attrs!=='object'){attrs={};}
tag=tag.toLowerCase();el=document.createElement(tag);for(k in attrs){v=attrs[k];el.setAttribute(k,v);}
el.appendChild(selRange.cloneContents());switch(mode){case 1:content=removeTag(el.innerHTML,tag);break;case 2:el.innerHTML=removeTag(el.innerHTML,tag);content=el.outerHTML;break;default:content=el.outerHTML;}
selRange.deleteContents();return pasteHtmlAtCaret(content,true);};clearTextWrapper=function(tags){var content,el,j,len1,tag;if(!selectedNode){return false;}
el=document.createElement(tag);el.appendChild(selRange.cloneContents());content=el.innerHTML;for(j=0,len1=tags.length;j<len1;j++){tag=tags[j];tag=tag.toLowerCase();content=removeTag(content,tag);}
selRange.deleteContents();return pasteHtmlAtCaret(content,true);};getElementStyle=function(element){return element.currentStyle||window.getComputedStyle(element,'');};pasteHtmlAtCaret=function(html,selectPastedContent){var el,firstNode,frag,lastNode,node,originalRange,range,sel;if(window.getSelection){sel=window.getSelection();if(sel.getRangeAt&&sel.rangeCount>0){range=sel.getRangeAt(0);range.deleteContents();el=document.createElement('div');el.innerHTML=html;frag=document.createDocumentFragment();node=void 0;lastNode=void 0;while(node=el.firstChild){lastNode=frag.appendChild(node);}
firstNode=frag.firstChild;range.insertNode(frag);if(lastNode){range=range.cloneRange();range.setStartAfter(lastNode);if(selectPastedContent){range.setStartBefore(firstNode);}else{range.collapse(true);}
sel.removeAllRanges();sel.addRange(range);}}}else if((sel=document.selection)&&sel.type!=='Control'){originalRange=sel.createRange();originalRange.collapse(true);sel.createRange().pasteHTML(html);if(selectPastedContent){range=sel.createRange();range.setEndPoint('StartToStart',originalRange);range.select();}}};removeInlineStyle=function(str){var regex;regex=new RegExp('(<[^>]+) style=["\'].*?["\']','ig');while(regex.test(str)){str=str.replace(regex,'$1');}
return str;};removeTag=function(str,tag){var reg;if(typeof tag!=='string'){return;}
tag=tag.toLowerCase();reg=new RegExp('(<'+tag+'\\s?.*?>)|(<\/\\s*'+tag+'\\s*>)','ig');return str.replace(reg,'');};placeCaretAfterNode=function(node){var range,selection;if(typeof window.getSelection==='undefined'||!node.parentNode){return;}
range=document.createRange();range.setStartAfter(node);range.collapse(true);selection=window.getSelection();selection.removeAllRanges();return selection.addRange(range);};getAllElementsFromSelection=function(){var allSelected,allWithinRange,el,i,j,len1,sel_cac;sel_cac=selRange.commonAncestorContainer;if(sel_cac.nodeType===1){allWithinRange=sel_cac.getElementsByTagName("*");}else{allWithinRange=[sel_cac.parentNode];}
allSelected=[];i=0;el=void 0;for(j=0,len1=allWithinRange.length;j<len1;j++){el=allWithinRange[j];if(selObj.containsNode(el,true)&&el.nodeType===1){allSelected.push(el);}
i+=1;}
return allSelected;};getSameParent=function(start,end){var new_end,new_start;new_start=start;while(new_start!==null){new_end=findSibling(new_start.parentNode,end);if(new_end){return{'point':new_start.parentNode,'start':new_start,'end':new_end};}else{new_start=new_start.parentNode;}}
return null;};findSibling=function(parent,child){var node;node=child;while(node!==null){if(node.parentNode===parent){return node;}
node=node.parentNode;}
return false;};findCellEl=function(element){var ref;if(!inEditor(element)){return null;}else if(ref=element.tagName.toLowerCase(),indexOf.call(CELL_TAGS,ref)>=0){return element;}else{return findCellEl(element.parentElement);}};smartFormatBlock=function(tag){var element;getCurrentSelection();if(inEditor(selectedNode,true)&&selectedOrigin.nodeType!==1){execFormatBlock(tag,true);getCurrentSelection();return historyManager.record();}else if(inEditor(selectedNode)){if(selectedNode.tagName!==tag.toUpperCase()){element=findCellEl(selectedNode);if(element){if(element.innerText!==''){element.innerHTML='<'+tag+'>'+element.innerText+'</'+tag+'>';placeCaretAfterNode(element.childNodes[0]);}}else{execFormatBlock(tag);}}else{if(selectedNode.parentNode.tagName==='DIV'){execFormatBlock('div');}else{selectedNode.outerHTML=selectedNode.innerHTML;placeCaretAfterNode(selectedNode);}}
getCurrentSelection();return historyManager.record();}};execFormatBlock=function(tag,force){if(typeof tag!=='string'){return;}
if(inEditor(selectedNode)||force){tag=tag.toLowerCase();if(startEl.parentElement===endEl.parentElement){document.execCommand('formatBlock',false,tag);return historyManager.record();}}};execCmd=function(cmd,show_ui,str){getCurrentSelection();if(!inEditor(selectedNode,true)){return;}
show_ui=Boolean(show_ui);if(typeof str!=='string'){str='';}
document.execCommand(cmd,show_ui,str);return historyManager.record();};destroySpan=function(element,br){var children,i,results;children=element.childNodes;i=0;results=[];while(i<children.length){if(children[i].tagName==='SPAN'&&children[i].getAttribute('style')){children[i].outerHTML=children[i].innerHTML;}else if(children[i].tagName==='BR'&&br){children[i].outerHTML='';}else{if(children[i].childNodes){destroySpan(children[i],br);}}
results.push(i+=1);}
return results;};getSelectionHtml=function(strip){var container,html,i,len,sel;html='';if(typeof window.getSelection!=='undefined'){sel=window.getSelection();if(!strip){if(sel.rangeCount){container=document.createElement('div');i=0;len=sel.rangeCount;while(i<len){container.appendChild(sel.getRangeAt(i).cloneContents());i+=1;}
html=container.innerHTML;}}else{html=sel.toString();}}else if(typeof document.selection!=='undefined'){if(document.selection.type==='Text'){html=document.selection.createRange().htmlText;}}
return html;};historyManager={redo:function(){if($history_step<($history.length-1)){$history_step+=1;return $editor.innerHTML=$history[$history_step];}},undo:function(){if($history_step>0){$history_step-=1;return $editor.innerHTML=$history[$history_step];}},reset:function(){$history=[$editor.innerHTML];return $history_step=0;},record:function(){if($editor.innerHTML===$history[$history_step||0]){return;}
if(typeof $history_step!=='number'){$history_step=0;}else{$history_step+=1;}
if($history.length>=($history_step+1)){$history.length=$history_step+1;}
if($history_step>=($options.history-1)){$history_step=$options.history-1;$history.shift();}
return $history.push($editor.innerHTML);}};log=function(args){var i;console.info('---------------------------');console.info('SAW Editor Log:',$property.name);for(i in arguments){if(arguments.hasOwnProperty(i)){console.info(arguments[i]);}}
return console.info('---------------------------');};errlog=function(args){var i;console.info('---------------------------');for(i in arguments){if(arguments.hasOwnProperty(i)){console.error('SAW Editor Error:',$property.name,arguments[i]);}}
return console.info('---------------------------');};dispatchSAWEvent=function(){var event;if(!$editor){return;}
event=new Event('saw');return $editor.dispatchEvent(event);};baseEventHandler=function(handler){return dispatchSAWEvent();};evtManager={register:function(target,handler,event){if(typeof event!=='string'){event='click';}
if(typeof target==='object'){target.addEventListener(event,handler);target.addEventListener(event,baseEventHandler);}},remove:function(target,handler,event){if(typeof event!=='string'){event='click';}
if(typeof target==='object'){if(typeof handler==='function'){target.removeEventListener(event,handler);target.removeEventListener(event,baseEventHandler);}}}};resetCurrentMedia=function(){$currentImage={link:void 0,element:void 0};$currentVideo={element:void 0};return $currentAudio={element:void 0};};resetCurrentLink=function(){return $currentLink={element:void 0};};resetCurrentPreset=function(){return $currentPreset={element:void 0};};doInsertImg=function(imgObj){var alt,class_,class_str,height,href,html,md_str,size_str,src,target,target_str,width;src=imgObj.src;alt=imgObj.title;href=imgObj.href;target=imgObj.target;width=imgObj.width;height=imgObj.height;class_=imgObj.class;if(class_){class_str='class="'+class_+'"';}else{class_str='';}
if(width||height){if(!width){width='auto';}
if(!height){height='auto';}
size_str='width="'+width+'" heigth="'+height+'"';}else{size_str='';}
if(href){if(target){target_str='target="'+target+'"';}else{target_str='';}
md_str='media-src="'+src+'"';html='<a '+md_str+' '+class_str+' '+target_str+' href="'+href+'">'+'<img src="'+src+'" '+size_str+' alt="'+alt+'"></a>';}else{html='<img '+class_str+' '+size_str+' src="'+src+'" alt="'+alt+'" />';}
return html;};doReplaceImg=function(imgObj){var alt,class_,class_str,height,href,img,src,target,target_str,width;img=$currentImage.element;if(!img||!inEditor(img)||!imgObj){return;}
src=imgObj.src;alt=imgObj.title;href=imgObj.href;target=imgObj.target;width=imgObj.width;height=imgObj.height;class_=imgObj.class;if(src){img.setAttribute('src',src);img.removeAttribute('ng-src');}
...
if(href){if($currentImage.link){$currentImage.link.setAttribute('href',href);$currentImage.link.removeAttribute('ng-href');if(target){$currentImage.link.setAttribute('target',target);}else{$currentImage.link.removeAttribute('target');}
if(class_){$currentImage.link.setAttribute('class',class_);}else{$currentImage.link.removeAttribute('class');}}else{if(target){target_str='target="'+target+'"';}else{target_str='';}
if(class_){class_str='class="'+class_+'"';}else{class_str='';}
img.outerHTML='<a '+class_str+' '+target_str+' href="'+href+'">'+'<img src="'+src+'" alt="'+alt+'"></a>';}}else{if($currentImage.link){$currentImage.link.outerHTML=$currentImage.link.innerHTML;}
if(class_){img.setAttribute('class',class_);}else{img.removeAttribute('class');}}
return img;}; doInsertVideo=function(videoObj){var autoplay,class_,class_str,controls,html,looping,poster,src;src=videoObj.src;poster=videoObj.poster;controls=videoObj.controls?'controls':'';looping=videoObj.loop?'loop':'';autoplay=videoObj.autoplay?'autoplay':'';class_=videoObj.class;if(class_){class_str='class="'+class_+'"';}else{class_str='';}
html='<video '+looping+' '+autoplay+' '+controls+' '+class_str+' '+'src="'+src+'" '+' poster="'+poster+'" preload="none"></video>';return html;};doReplaceVideo=function(videoObj){var autoplay,class_,controls,looping,poster,src,video;video=$currentVideo.element;if(!video||!inEditor(video)||!videoObj){return;}
src=videoObj.src;poster=videoObj.poster;controls=videoObj.controls?'controls':'';looping=videoObj.loop?'loop':'';autoplay=videoObj.autoplay?'autoplay':'';class_=videoObj.class;if(src){video.setAttribute('src',src);video.removeAttribute('ng-src');}
if(class_){video.setAttribute('class',class_);}else{video.removeAttribute('class');}
if(poster){video.setAttribute('poster',poster);}else{video.removeAttribute('poster');}
if(controls){video.setAttribute('controls',true);}else{video.removeAttribute('controls');}
if(looping){video.setAttribute('loop',true);}else{video.removeAttribute('loop');}
if(autoplay){video.setAttribute('autoplay',true);}else{video.removeAttribute('autoplay');}
return video;};doInsertAudio=function(audioObj){var autoplay,class_,class_str,controls,html,looping,src;src=audioObj.src;controls='controls';looping=audioObj.loop?'loop':'';autoplay=audioObj.autoplay?'autoplay':'';class_=audioObj.class;if(class_){class_str='class="'+class_+'"';}else{class_str='';}
html='<audio '+looping+' '+autoplay+' '+controls+' '+class_str+' '+'src="'+src+'"></audio>';return html;};doReplaceAudio=function(audioObj){var audio,autoplay,class_,controls,looping,src;audio=$currentAudio.element;if(!audio||!inEditor(audio)||!audioObj){return;}
src=audioObj.src;controls=audioObj.controls?'controls':'';looping=audioObj.loop?'loop':'';autoplay=audioObj.autoplay?'autoplay':'';class_=audioObj.class;if(src){audio.setAttribute('src',src);audio.removeAttribute('ng-src');}
if(class_){audio.setAttribute('class',class_);}else{audio.removeAttribute('class');}
if(controls){audio.setAttribute('controls',true);}else{audio.removeAttribute('controls');}
if(looping){audio.setAttribute('loop',true);}else{video.removeAttribute('loop');}
if(autoplay){audio.setAttribute('autoplay',true);}else{audio.removeAttribute('autoplay');}
return audio;};execInsertMedia=function(mediaObj){var html;if(!savedRange||!mediaObj){return;}
if(mediaObj.type==='video'){html=doInsertVideo(mediaObj);}else if(mediaObj.type==='audio'){html=doInsertAudio(mediaObj);}else{html=doInsertImg(mediaObj);}
restoreSelection(savedRange);pasteHtmlAtCaret(html);historyManager.record();return dispatchSAWEvent();};execRemoveMedia=function(mediaObj){var link,media;if(mediaObj.type==='video'){media=$currentVideo.element;link=null;}else if(mediaObj.type==='audio'){media=$currentAudio.element;link=null;}else{media=$currentImage.element;link=$currentImage.link;}
if(!media||!inEditor(media)){return;}
media.outerHTML='';if(link){link.outerHTML='';}
resetCurrentMedia();historyManager.record();return dispatchSAWEvent();};execReplaceMedia=function(mediaObj){var media;if(mediaObj.type==='video'){media=doReplaceVideo(mediaObj);}else if(mediaObj.type==='audio'){media=doReplaceAudio(mediaObj);}else{media=doReplaceImg(mediaObj);}
resetCurrentMedia();historyManager.record();return dispatchSAWEvent();};execInsertLink=function(linkObj){var a,class_,class_str,href,link,name,target,target_str;if(!savedRange||!linkObj){return;}
href=linkObj.href;name=linkObj.name||href;target=linkObj.target;class_=linkObj.class;restoreSelection(savedRange);if(savedRange.endOffset!==savedRange.startOffset){execCmd('createLink',false,href);getCurrentSelection();a=selectedNode;if(a.tagName==='A'){if(target){a.setAttribute('target',target);}
if(class_){a.setAttribute('class',class_);}}}else{if(target){target_str='target="'+target+'"';}else{target_str='';}
if(class_){class_str='class="'+class_+'"';}else{class_str='';}
link='<a '+class_str+' '+target_str+' href="'+href+'">'+name+'</a>';pasteHtmlAtCaret(link);}
historyManager.record();return dispatchSAWEvent();};execReplaceLink=function(linkObj){var a,class_,href,target;a=$currentLink.element;if(!a||!inEditor(a)||!linkObj){return;}
href=linkObj.href;target=linkObj.target;class_=linkObj.class;restoreSelection(savedRange);a.removeAttribute('ng-href');if(href){a.setAttribute('href',href);}
if(class_){a.setAttribute('class',class_);}else{a.removeAttribute('class');}
if(target){a.setAttribute('target',target);}else{a.removeAttribute('target');}
resetCurrentLink();historyManager.record();return dispatchSAWEvent();};execRemoveLink=function(){var a;a=$currentLink.element;if(!a||!inEditor(a)){return;}
a.outerHTML=a.innerHTML;resetCurrentLink();historyManager.record();return dispatchSAWEvent();};execInsertPreset=function(preset){var element,preset_element;if(!preset){return;}
preset_element=document.createElement('div');if(preset.controls){preset_element.setAttribute('preset',preset.type);}
preset_element.innerHTML=preset.rendered||'';if(savedRange){restoreSelection(savedRange);}
element=findTopElement(selectedNode);if(element&&element.nextSibling){$editor.insertBefore(preset_element,element.nextSibling);}else{$editor.appendChild(preset_element);}
historyManager.record();return dispatchSAWEvent();};execReplacePreset=function(preset){var preset_element;preset_element=$currentPreset.element;if(!preset_element||!inEditor(preset_element)){return;}
if(preset.controls){preset_element.setAttribute('preset',preset.type);}else{preset_element.removeAttribute('preset');}
preset_element.innerHTML=preset.rendered||'';resetCurrentPreset();historyManager.record();return dispatchSAWEvent();};execRemovePreset=function(){var div;div=$currentPreset.element;if(!div||!inEditor(div)){return;}
div.removeAttribute('preset');div.innerHTML='&nbsp;';resetCurrentPreset();historyManager.record();return dispatchSAWEvent();};execInsertScript=function(script){if(!savedRange||!script){return;}
if(typeof script!=='string'){script='';}
restoreSelection(savedRange);pasteHtmlAtCaret(script);historyManager.record();return dispatchSAWEvent();};init_editor=function(editor,toolbar,opt){var _history,e,style_el;load_options(opt);if(typeof editor==='string'&&editor!==''){$editor=document.querySelector('[id='+editor+']');}else if(isHTMLElement(editor)){$editor=editor;}
if(!$editor){return errlog('Init editor failed !!');}
if(typeof toolbar==='string'){$toolbar=document.querySelector('[id='+toolbar+']');}
if(isHTMLElement(toolbar)){$toolbar=toolbar;}else{console.warn('SAW Editor: Editor is not toolbar ...');}
if(typeof $options.history!=='number'){try{_history=parseInt($options.history);$options.history=Math.min(Math.max(_history,100),10);}catch(error){e=error;$options.history=100;}}
style_el=document.createElement('style');style_el.setAttribute('saw-style','');style_el.innerHTML=['sup-angular-wysiwyg [preset],','[sup-angular-wysiwyg] [preset] ','{cursor: pointer;}','sup-angular-wysiwyg [preset] [prevent-touch],','[sup-angular-wysiwyg] [preset] [prevent-touch] ','{pointer-events: none;}'].join(' ');document.head.appendChild(style_el);$editor.setAttribute('contenteditable','true');$property.id=$editor.getAttribute('id')||Date.now();$property.order=parseInt($editor.getAttribute('order'))||0;$property.name=$editor.getAttribute('name')||now;$editor.addEventListener('mousedown',focusHandler);$editor.addEventListener('click',runtimeHandler);$editor.addEventListener('keyup',runtimeHandler);$editor.addEventListener('dblclick',doubleClickHandler);$editor.addEventListener('paste',pasteHandler);if($toolbar){$toolbar.addEventListener('mousedown',focusHandler);}
window.addEventListener('mousedown',blurHandler);set_toolbar();set_keyboard();historyManager.record();console.info('supAngularWysiwyg('+ver+'): '+$property.name+' inited');};destroy_editor=function(){$editor.removeEventListener('mousedown',focusHandler);$editor.removeEventListener('click',runtimeHandler);$editor.removeEventListener('keyup',runtimeHandler);$editor.removeEventListener('dbclick',doubleClickHandler);$editor.removeEventListener('paste',pasteHandler);if($toolbar){$toolbar.removeEventListener('mousedown',focusHandler);}
window.removeEventListener('mousedown',blurHandler);unset_toolbar();unset_keyboard();console.info('supAngularWysiwyg: destroyed');};set_toolbar=function(){var _handler,btn,btn_els,el,idx,j,len1,results;if(!$toolbar){return;}
results=[];for(j=0,len1=BTN_LIST.length;j<len1;j++){btn=BTN_LIST[j];btn_els=$toolbar.querySelectorAll('[saw-tool-btn="'+btn.name+'"]');results.push((function(){var l,len2,results1;results1=[];for(idx=l=0,len2=btn_els.length;l<len2;idx=++l){el=btn_els[idx];_handler=eval(btn.handler);evtManager.register(el,_handler);results1.push($btn_elements_stack.push({element:el,handler:_handler}));}
return results1;})());}
return results;};unset_toolbar=function(){var btn,j,len1,results;if(!$toolbar){return;}
results=[];for(j=0,len1=$btn_elements_stack.length;j<len1;j++){btn=$btn_elements_stack[j];results.push(evtManager.remove(btn.element,btn.handler));}
return results;};set_keyboard=function(){$editor.addEventListener('keydown',keyboardHandler);return $editor.addEventListener('keyup',keyboardHandler);};unset_keyboard=function(){$editor.removeEventListener('keydown',keyboardHandler);return $editor.removeEventListener('keyup',keyboardHandler);};load_options=function(opt){var key,results;if(typeof opt!=='object'){return;}
results=[];for(key in $options){if(key in opt){results.push($options[key]=opt[key]);}else{results.push(void 0);}}
return results;};set_hook=function(hook_name,func){if(typeof func!=='function'||!(hook_name in hooks)){return;}
return hooks[hook_name]=func;};expose={init:init_editor,destroy:destroy_editor,insertMedia:execInsertMedia,replaceMedia:execReplaceMedia,removeMedia:execRemoveMedia,insertLink:execInsertLink,replaceLink:execReplaceLink,removeLink:execRemoveLink,insertPreset:execInsertPreset,replacePreset:execReplacePreset,removePreset:execRemovePreset,insertScript:execInsertScript,setHook:set_hook};Object.defineProperty(expose,'version',{get:function(){return ver;}});Object.defineProperty(expose,'id',{get:function(){return $property.id;}});Object.defineProperty(expose,'order',{get:function(){return $property.order;}});Object.defineProperty(expose,'element',{get:function(){return $editor;}});Object.defineProperty(expose,'options',{get:function(){return $options;}});return expose;};angular.module('supAngularWysiwyg',[]).directive('supAngularWysiwyg',['$rootScope','$window','$interpolate','$timeout',function($rootScope,$window,$interpolate,$timeout){return{restrict:'EA',require:'?ngModel',scope:{toolbar:'@',history:'@',default:'@',embedMode:'@',readonly:'@',presets:'=',textStyles:'='},link:function(scope,element,attrs,ngModel){var default_data,dirty,e,editor,expose,j,l,len1,len2,options,p,presets,readViewText,ref,ref1,root,st,text_styles,toolbar;if(!ngModel){return;}
scope.embedMode=angular.isDefined(scope.embedMode);scope.readonly=angular.isDefined(scope.readonly);editor=new supWysiwygEditor;toolbar=scope.toolbar||'saw-editor-toolbar';try{options={history:parseInt(scope.history)};}catch(error){e=error;options={};}
presets=[];if(angular.isArray(scope.presets)){ref=scope.presets;for(j=0,len1=ref.length;j<len1;j++){p=ref[j];presets.push({name:p.name,type:p.type,figure:p.figure,code:p.code});}}
text_styles=[];if(angular.isArray(scope.textStyles)){ref1=scope.textStyles;for(l=0,len2=ref1.length;l<len2;l++){st=ref1[l];if(!st.class){continue;}
text_styles.push({name:st.name,class:st.class,colorize:st.colorize||''});}}
root=$rootScope;if(!scope.embedMode&&$window.parent.document!==$window.document){root=$window.parent.angular.element($window.parent.document).scope();toolbar=$window.parent.document.querySelector('[id='+toolbar+']');}
expose={presets:presets,text_styles:text_styles};root.$emit('saw.ready',expose,editor);$timeout(function(){var loaded;if(scope.readonly){editor='readonly';}else{editor.init(element[0],toolbar,options);}
loaded={label:attrs.label||attrs.name,model:attrs.ngModel};root.$emit('saw.loaded',loaded,editor);if(root!==$rootScope){return $rootScope.$emit('saw.loaded',loaded,editor);}},0);editor.setHook('selText',function(el){return root.$emit('saw.select_text',editor,el);});editor.setHook('selList',function(el){return root.$emit('saw.select_list',editor,el);});editor.setHook('insertLink',function(has_selection){return root.$emit('saw.insert_link',editor,has_selection);});editor.setHook('editLink',function(link){return root.$emit('saw.edit_link',editor,link);});editor.setHook('insertMedia',function(){return root.$emit('saw.insert_media',editor);});editor.setHook('editMedia',function(media){return root.$emit('saw.edit_media',editor,media);});editor.setHook('insertPreset',function(){return root.$emit('saw.insert_preset',editor,presets);});editor.setHook('editPreset',function(curr_preset){return root.$emit('saw.edit_preset',editor,presets,curr_preset);});editor.setHook('insertScript',function(){return root.$emit('saw.insert_script',editor);});editor.setHook('focus',function(){return root.$emit('saw.focus',editor);});editor.setHook('blur',function(){return root.$emit('saw.blur',editor);});editor.setHook('openHTML',function(){return root.$emit('saw.open_html',editor,ngModel.$viewValue);});dirty=false;if(scope.default){default_data=scope.default;}else{default_data=$interpolate(element.html())($rootScope);}
readViewText=function(){var html;html=element.html();if(html==="<br>"){html="";}
return ngModel.$setViewValue(html);};ngModel.$render=function(){var timer;if(!ngModel.$viewValue&&!dirty){element.html(default_data);timer=$timeout(function(){readViewText();return $timeout.cancel(timer);},0);}else{element.html(ngModel.$viewValue||'');}
return dirty=true;};element.on('blur keyup change mousedown cut paste saw',function(e){if(scope.readonly){return;}
readViewText();});return scope.$on('$destroy',function(){return editor.destroy();});}};}]);}).call(this);(function(){var LOCALIZED_DICT,is_exports,root;is_exports=typeof exports!=="undefined"&&exports!==null;root=is_exports?exports:this;LOCALIZED_DICT={};root.supLocales=function(opts){var _load_dict,_trans_key,append,i,insert,key,len,locale,localizedText,methods,options,restore,self_translate,set_locale,translate,ver;ver='1.4.0';localizedText={};locale=null;options={case_sensitive:false};if(typeof opts==='object'&&opts!==null){for(i=0,len=optinos.length;i<len;i++){key=optinos[i];if(key in opts){options[key]=opts[key];}}}
_trans_key=function(str){if(!options.case_sensitive){return str.toLowerCase();}
return str;};_load_dict=function(loc_texts){var j,len1,loc_text_dict,text;if(!Array.isArray(loc_texts)||!loc_texts.length){return{};}
loc_text_dict={};for(j=0,len1=loc_texts.length;j<len1;j++){text=loc_texts[j];if(text.msgid){loc_text_dict[_trans_key(text.msgid)]=text.msgstr;}}
return loc_text_dict;};insert=function(loc,loc_texts){if(loc&&Array.isArray(loc_texts)){return LOCALIZED_DICT[loc.toString()]=loc_texts;}};append=function(new_text_list){var j,len1,text;if(!Array.isArray(new_text_list)){return;}
for(j=0,len1=new_text_list.length;j<len1;j++){text=new_text_list[j];if(text.msgid){localizedText[_trans_key(text.msgid)]=text.msgstr;}}};restore=function(){set_locale(locale);};set_locale=function(loc){var k,locale_dict,v;if(!loc){loc='';}
locale=null;loc=loc.replace('-','_');for(k in LOCALIZED_DICT){v=LOCALIZED_DICT[k];if(k.toLowerCase()===loc.toLowerCase()){locale=k;break;}}
if(locale&&LOCALIZED_DICT[locale]){locale_dict=LOCALIZED_DICT[locale];}else{locale_dict=null;locale=loc;}
localizedText=_load_dict(locale_dict);return locale;};translate=function(text){var arg,args,j,l,len1,len2,ref,trans;if(!text||typeof text!=='string'){return text;}
trans=localizedText[_trans_key(text)];if(!trans){trans=text;}
args=[];for(j=0,len1=arguments.length;j<len1;j++){arg=arguments[j];args.push(arg);}
ref=args.slice(1);for(l=0,len2=ref.length;l<len2;l++){arg=ref[l];if(arg===void 0){arg='';}
trans=trans.replace("%s",arg);}
return trans;};self_translate=function(text){var lang,trans_text;if(!text||typeof text!=='object'){return text;}
if(locale){lang=locale.split('_')[0]||'en';trans_text=text[locale]||text[lang];}
if(!trans_text){trans_text=text['en_US']||text['en'];}
if(!trans_text){key=Object.keys(text)[0];trans_text=text[key];}
return trans_text;};methods={version:ver,locale:locale,options:options,translate:translate,self_translate:self_translate,set_locale:set_locale,restore:restore,append:append,insert:insert};return methods;};supLocales.DICT=LOCALIZED_DICT;angular.module('supLocales',['ngCookies']).run(['supLocales',function(supLocales){if(!supLocales.inited){return supLocales.init();}}]).service("supLocales",['$location','$rootScope','$cookies',function($location,$rootScope,$cookies){var default_locale,self,sup_locales;default_locale='en_US';sup_locales=new supLocales();self=this;this.version=sup_locales.version;this.inited=false;this.cookie_opts={path:'',domain:'',expires:new Date(Date.now()+3600*24*365*1000),secure:false};this.init=function(loc,opts){var cookieLocale,curr_locale,e,key,params,userLang,userLocale;if(angular.isObject(opts)){for(key in self.cookie_opts){if(key in opts){self.cookie_opts[key]=opts[key];}}
for(key in sup_locales.options){if(key in opts){sup_locales.options[key]=opts[key];}}}
if(loc){default_locale=loc;}
userLang=navigator.language||navigator.userLanguage;userLocale=userLang.replace('-','_');try{cookieLocale=$cookies.get('current_locale');}catch(error){e=error;cookieLocale=null;}
params=$location.search();if(params.locale){curr_locale=params.locale;$location.search("locale",null);}else{curr_locale=cookieLocale?cookieLocale:userLocale;}
self.set(curr_locale);$rootScope._=angular.translate=sup_locales.translate;return this.inited=true;};this.load=function(loc,text_list){sup_locales.insert(loc,text_list);return sup_locales.set_locale(loc);};this.translate=function(args){return sup_locales.translate.apply(this,arguments);};this.set=function(loc){var locale;locale=sup_locales.set_locale(loc)||default_locale;$rootScope.locale=locale;$rootScope.lang=locale.split('_')[0];return $cookies.put('current_locale',locale,self.cookie_opts);};this.get=function(output_lang){var _locale,cookieLocale,e;try{cookieLocale=$cookies.get('current_locale');}catch(error){e=error;cookieLocale=null;}
_locale=cookieLocale||sup_locales.locale||default_locale;if(output_lang){return _locale.split('_')[0];}else{return _locale;}};this.append=function(translate_text_list){return sup_locales.append(translate_text_list);};this.restore=function(){return sup_locales.restore();};this.match=function(lang1,lang2){if(typeof lang1!=='string'||typeof lang2!=='string'){return false;}
lang1=lang1.replace('-','_');lang2=lang2.replace('-','_');return lang1.toLowerCase()===lang2.toLowerCase();};return this;}]);}).call(this);(function(){var _add_url_param,_strip,_url_parse_qs,indexOf=[].indexOf;angular.browser=function(){var browser,is_mobile,re,ua;ua=navigator.userAgent;re=/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i;is_mobile=re.test(ua)||ua.indexOf('Mobile')>-1;browser={user_agent:ua,is_mobile:is_mobile};return browser;};angular.uuid4=function(hex){var output,s4;s4=function(){return Math.floor((1+Math.random())*0x10000).toString(16).substring(1);};if(hex===void 0){hex=true;}
if(hex){output=s4()+s4()+s4()+s4()+s4()+s4()+s4()+s4();}else{output=s4()+s4()+'-'+s4()+'-'+s4()+'-'+s4()+'-'+s4()+s4()+s4();}
return output;};angular.iter_product=function(args){return args.reduce(function(x,y){var tmp;tmp=[];x.forEach(function(i){return y.forEach(function(j){return tmp.push(i.concat(j));});});return tmp;},[[]]);};angular.escape_asterisk=function(input,asterisk,output){if(asterisk===void 0){asterisk='*';}
if(output===void 0){output=null;}
if(input===asterisk){return output;}else{return input;}};angular.list=function(param,_default){if(_default===void 0){_default=[];}else if(!angular.isArray(_default)){_default=[_default];}
if(param){if(angular.isArray(param)){return param;}else{return[param];}}else{return _default;}};angular.bool=function(param,_default){if(_default===void 0){_default=true;}else{_default=Boolean(_default);}
if(param===void 0){return _default;}else{return Boolean(param);}};angular.round_fractional=function(num,dig){return Math.round(num*Math.pow(10,dig))/Math.pow(10,dig);};angular.parse_int=function(num,_default,_natural){if(_default===void 0){_default=0;}
if(_natural===void 0){_natural=false;}
_default=parseInt(_default)||0;if(typeof _natural!=='boolean'){_natural=parseInt(_natural)||0;}
try{num=parseInt(num);if(isNaN(num)){num=_default;}}catch(error1){num=_default;}
if(_natural===0){num=Math.max(num,0);}else if(_natural){num=Math.max(num,1);}
return num;};angular.str_byte=function(str){var _len,charCode,i;if(typeof str!=='string'){return null;}
_len=0;charCode=void 0;i=0;while(i<str.length){charCode=str.charCodeAt(i);if(charCode<0x007f){_len++;}else if(0x0080<=charCode&&charCode<=0x07ff){_len+=2;}else if(0x0800<=charCode&&charCode<=0xffff){_len+=3;}else{_len+=4;}
i+=1;}
return _len;};angular.pop=function(obj,key){var value;value=obj[key];delete obj[key];return value;};angular.set=function(list){var i,l,len1,newlist;newlist=[];for(l=0,len1=list.length;l<len1;l++){i=list[l];if(indexOf.call(newlist,i)<0){newlist.push(i);}}
return newlist;};angular.get_file_ext=function(str){var e,ext,pair;try{str=str.split('?')[0].split('#')[0];if(str.substr(-1)==='/'){str=str.substr(0,str.length-1);}
pair=str.split('.');if(pair.length>1){ext=pair.pop();return ext.toLowerCase();}
return'';}catch(error1){e=error1;return'';}}; angular.pre_process_slug=function(value){var slug;try{slug=value.toLowerCase();if(!slug.match(/^[a-z0-9_\-]+$/g)){return null;}}catch(error1){slug=null;}
return slug;};angular.strip=function(str,text){return _strip(str,text,true,true);};angular.lstrip=function(str,text){return _strip(str,text,true,false);};angular.rstrip=function(str,text){return _strip(str,text,false,true);};_strip=function(str,text,left,right){var regex_left,regex_right;if(typeof str!=='string'){return null;}
...
return str;};angular.startswith=function(str,text){if(typeof str!=='string'||typeof text!=='string'){return null;}
return str.indexOf(text)===0;};angular.endswith=function(str,text){if(typeof str!=='string'||typeof text!=='string'){return null;}
return str.substr(str.length-text.length)===text;};angular.ellipsis=function(str,digit,suffix){var len;digit=parseInt(digit);if(!angular.isString(suffix)){suffix='...';}
if(!digit){digit=str.length-suffix.length;}
len=str.length;str=str.substr(0,digit);if(str.length<len){str+=suffix;}
return str;};angular.currency=function(amount,symbol,fractionSize){var size;if(symbol===void 0){symbol='';}
size=parseInt(fractionSize)||2;amount=amount.toFixed(size).replace(/(\d)(?=(\d{3})+\.)/g,'$1,');return symbol+amount;};angular.price=function(amount,use_currency,fractionSize){var price,size;size=parseInt(fractionSize)||2;price=(parseInt(amount)/100)||0;if(use_currency){price=price.toFixed(size).replace(/(\d)(?=(\d{3})+\.)/g,'$1,');}else{price=price.toFixed(size);}
return price;};angular.now=function(){return parseInt(Date.now()/1000);};angular.timestamp2date=function(timestamp,failback){var date,fail;timestamp=parseInt(timestamp);if(timestamp.toString().length===10){timestamp=timestamp*1000;}
try{date=new Date(timestamp);if(isNaN(date)){throw'Invaild Date';}}catch(error1){date=new Date();if(typeof failback==='function'){fail=failback(date);if(typeof fail.getDate==='function'){date=fail;}}}
return date;};angular.date2timestamp=function(date,unix){var _stamp_base;if(unix===void 0){unix=true;}
_stamp_base=unix?1000:1;return Math.ceil(date.getTime()/_stamp_base);};angular.date2str=function(date,format){var date_str,dd,hour,mins,mm,secs,yyyy;if(!angular.isDate(date)){return'';}
if(!format){format='YYYY-MM-DD';}
dd=date.getDate();mm=date.getMonth()+1;yyyy=date.getFullYear();hour=date.getHours();mins=date.getMinutes();secs=date.getSeconds();if(dd<10){dd='0'+dd;}
if(mm<10){mm='0'+mm;}
if(hour<10){hour='0'+hour;}
if(mins<10){mins='0'+mins;}
if(secs<10){secs='0'+secs;}
date_str=format.replace('YYYY',yyyy).replace('MM',mm).replace('DD',dd).replace('hh',hour).replace('mm',mins).replace('ss',secs);return date_str;};angular.str2date=function(date_str){var date,e;if(typeof date_str!=='string'){return false;}
try{date=new Date(date_str);}catch(error1){e=error1;return false;}
if(!date.getTime()){return false;}
return date;};angular.sortedby=function(source,sort_keys,reverse){var compare,key,l,len1,new_list,parse_sorts,sorts;sorts=[];if(!angular.isArray(sort_keys)){sort_keys=[sort_keys];}
parse_sorts=function(key){var direction;if(angular.isArray(key)){return sorts.push([key[0],key[1]]);}else if(angular.isString(key)){if(key.charAt(0)==='-'){key=key.replace(/^\-*/,'');direction=-1;}else{key=key.replace(/^\+*/,'');direction=1;}
return sorts.push([key,direction]);}};for(l=0,len1=sort_keys.length;l<len1;l++){key=sort_keys[l];parse_sorts(key);}
compare=function(a,b){var direction,len2,m,sort;for(m=0,len2=sorts.length;m<len2;m++){sort=sorts[m];key=sort[0];direction=sort[1];if(a[key]<b[key]){return-1*direction;}
if(a[key]>b[key]){return 1*direction;}}
return 0;};new_list=angular.copy(source).sort(compare);if(reverse){new_list.reverse();}
return new_list;};angular.capitalize=function(str){var e;try{return str.charAt(0).toUpperCase()+str.slice(1).toLowerCase();}catch(error1){e=error1;return'';}}; angular.setdefault=function(dict,key,_default){if(dict[key]===void 0){dict[key]=_default;}
return dict;};angular.str2opts=function(opts_str,separator,exclude_mark){var _key,e,l,len1,op,op_key,opts,opts_list;if(typeof opts_str!=='string'){return false;}
if(typeof separator!=='string'||!separator){separator=',';}
if(typeof exclude_mark!=='string'||!exclude_mark){exclude_mark='!';}
try{opts_list=opts_str.split(separator);}catch(error1){e=error1;opts_list=[];}
opts={};for(l=0,len1=opts_list.length;l<len1;l++){op=opts_list[l];_key=op.trim();op_key=_key.replace(exclude_mark,'');if(_key){opts[op_key]=Boolean(_key.indexOf(exclude_mark)!==0);}}
return opts;};angular.list2dict=function(list,key,key2){var dict,idx,item,l,len1,value;if(typeof key!=='string'){key=null;}
if(typeof key2!=='string'){key2=null;}
dict={};for(idx=l=0,len1=list.length;l<len1;idx=++l){item=list[idx];value=key2?item[key2]:item;if(key){if(key in item){dict[item[key]]=value;}}else{dict[idx.toString()]=value;}}
return dict;};angular.str2list=function(str,is_set,separator,whitespace){var e,l,len1,newlist,orglist,s;if(angular.isArray(str)){return str;}else if(typeof str!=='string'){return[];}
if(!separator){separator=',';str=str.replace(/[，]/g,',');}
try{orglist=str.split(separator);}catch(error1){e=error1;orglist=[];}
newlist=[];for(l=0,len1=orglist.length;l<len1;l++){s=orglist[l];if(s){if(!whitespace){s=s.trim();}
if(is_set&&indexOf.call(newlist,s)>=0){continue;}
newlist.push(s);}}
return newlist;};angular.list2str=function(list,separator){var e,str;if(typeof list==='string'){return list;}else if(!angular.isArray(list)){return'';}
if(!separator){separator=', ';}
try{str=list.join(separator);}catch(error1){e=error1;str='';}
return str;};angular.list_index=function(list,item,attr){var equals,idx,l,len1,obj;equals=function(obj1,obj2,attr){if(attr){return angular.equals(obj1[attr],obj2[attr]);}else{return angular.equals(obj1,obj2);}};for(idx=l=0,len1=list.length;l<len1;idx=++l){obj=list[idx];if(equals(obj,item,attr)){return idx;}}
return null;};angular.list_shift=function(list,curr_index,next_index,switching){var _tmp;if(switching===void 0){switching=true;}
...
return list;};angular.list_pop=function(list,item,attr){var equals,idx,l,len1,obj,pop_obj;equals=function(obj1,obj2,attr){if(attr){return angular.equals(obj1[attr],obj2[attr]);}else{return angular.equals(obj1,obj2);}};pop_obj=null;for(idx=l=0,len1=list.length;l<len1;idx=++l){obj=list[idx];if(equals(obj,item,attr)){pop_obj=obj;list.splice(idx,1);break;}}
return pop_obj;};angular.list_remove=function(list,item,attr){var equals,i,idx,indexes,l,len1,len2,m,obj;equals=function(obj1,obj2,attr){if(attr){return angular.equals(obj1[attr],obj2[attr]);}else{return angular.equals(obj1,obj2);}};indexes=[];for(idx=l=0,len1=list.length;l<len1;idx=++l){obj=list[idx];if(equals(obj,item,attr)){indexes.unshift(idx);}}
for(m=0,len2=indexes.length;m<len2;m++){i=indexes[m];list.splice(i,1);}
return list;};angular.update_dict=function(obj,obj2){var k,v;for(k in obj2){v=obj2[k];obj[k]=v;}
return obj;}; angular.format_tags=function(tags,limit,upper){var _tag,_tags,l,len1,styl,tag;if(!angular.isArray(tags)){return[];}
if(typeof limit!=='number'||!limit){limit=60;}
styl=function(text){if(upper){return angular.upper(text);}else{return angular.lower(text);}};_tags=[];for(l=0,len1=tags.length;l<len1;l++){tag=tags[l];if(typeof tag!=='string'){continue;}
_tag=styl(tag);if(_tags.indexOf(_tag)<0){_tags.push(_tag);}}
return _tags.slice(0,limit);};angular.obj2jsonstr=function(obj,ugly){var e,i,jsonstr,prop;jsonstr=null;if(!obj){return'';}
if(typeof obj==="string"){return obj;}else if(typeof obj!=="object"){return null;}
i=0;for(prop in obj){if(obj.hasOwnProperty(prop)){i+=1;}}
if(i===0){jsonstr='';}else{try{if(!ugly){jsonstr=JSON.stringify(obj,null,2);}else{jsonstr=JSON.stringify(obj);}}catch(error1){e=error1;jsonstr=false;}}
return jsonstr;};angular.jsonstr2obj=function(str,parser){var e,obj;if(!str){return{};}
obj=null;if(typeof parser!=='function'){parser=JSON.parse;}
try{obj=parser(str);}catch(error1){e=error1;obj=false;}
return obj;}; angular.title=function(text){return text.charAt(0).toUpperCase()+text.slice(1).toLowerCase();};angular.lower=function(text){return text.toLowerCase();};angular.upper=function(text){return text.toUpperCase();};angular.clean_tags=function(str){if(!angular.isString(str)){str='';}
return str.replace(/<[^>]+>/gm,'');};angular.clean_space=function(content,force){if(force){return content.replace(" ","").trim();}else{return content.replace(/\s{2,}/g," ").trim();}};angular.clean_ng_attr=function(content){return content.replace(/ng-[a-zA-Z0-9_\-]+=["\'].*?["\']/ig,'');};angular.cleanup_html=function(html_content){var content,dummy;content=html_content.replace(/ng-[a-zA-Z0-9_\-]+=["\'].*?["\']/ig,'');dummy=document.createElement('div');dummy.innerHTML=content;return dummy.innerHTML;};angular.isDict=function(obj){return Boolean(angular.isObject(obj)&&!angular.isArray(obj));};angular.isUrl=function(url){var regex;if(typeof url!=='string'){return false;}
regex=/^([\w]+:)?\/\/[a-zA-Z0-9]/i;return Boolean(url.match(regex));};angular.isUrlScheme=function(str){var regex;if(typeof str!=='string'){return false;}
regex=/^[\w]+:/i;return Boolean(str.match(regex));};angular.addURLParams=function(url,input_params,unique,concat){var k,params,url_hash,v;if(concat===void 0){concat=true;}
if(unique===void 0){unique=true;}
if(angular.isObject(input_params)){for(k in input_params){v=input_params[k];if(!angular.isArray(v)){input_params[k]=[v];}}}else if(angular.isString(input_params)){input_params={input_params:['']};}else{return'';}
params=_url_parse_qs(url);url_hash=indexOf.call(url,'#')>=0?url.substring(url.indexOf('#'),url.length)||'':'';url=url.split('?')[0].split('#')[0];if(concat){for(k in input_params){v=input_params[k];if(k in params){if(angular.isArray(params[k])){params[k]=params[k].concat(v);}else{params[k]=[params[k]].concat(v);}}else{params[k]=v;}}}else{params=input_params;}
if(unique){for(k in params){v=params[k];params[k]=v[v.length-1];}}
for(k in params){v=params[k];url=_add_url_param(url,k,v);}
url=angular.strip(url.replace('=&','&'),'=');return url+url_hash;};angular.getURLParams=function(url,unique){var k,params,v;if(unique===void 0){unique=true;}
params=_url_parse_qs(url);if(unique){for(k in params){v=params[k];params[k]=v[v.length-1];}}
return params;};_add_url_param=function(url,key,value){var _add,item,l,len1;_add=function(url,key,value){var joint;joint=url.indexOf('?')>-1?'&':'?';url+=joint+encodeURIComponent(key)+'='+encodeURIComponent(value);return url;};if(angular.isArray(value)){for(l=0,len1=value.length;l<len1;l++){item=value[l];url=_add(url,key,item);}}else{url=_add(url,key,value);}
return url;};_url_parse_qs=function(url){var k,l,len1,pair,params,query_idx,query_vars,v;if(typeof url!=='string'){return{};}
if(indexOf.call(url,'#')>=0){url=url.substring(0,url.indexOf('#'));}
params={};query_idx=url.indexOf('?');if(query_idx>-1){query_vars=url.split('?')[1].split('&');for(l=0,len1=query_vars.length;l<len1;l++){v=query_vars[l];if(!v){continue;}
pair=v.split('=');k=pair[0];v=pair[1]||'';if(params[k]){params[k].push(v);}else{params[k]=[v];}}}
return params;};angular.getPureURL=function(url){try{return url.split(/[?#]/)[0];}catch(error1){return'';}}; angular.dataURLToBlob=function(dataURL){var BASE64_MARKER,contentType,i,parts,raw,rawLength,uInt8Array;BASE64_MARKER=';base64,';if(dataURL.indexOf(BASE64_MARKER)===-1){parts=dataURL.split(',');contentType=parts[0].split(':')[1];raw=parts[1];return new Blob([raw],{type:contentType});}
...
return new Blob([uInt8Array],{type:contentType});};angular.esc_regexp=function(str){return str.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');};angular.safe_apply=function(scope){var e;try{if(scope.$root.$$phase!=='$apply'&&scope.$root.$$phase!=='$digest'){return scope.$apply();}}catch(error1){e=error1;}};if(typeof angular.translate!=='function'){angular.translate=function(str){var arg,args,l,len1,len2,m,ref;if(typeof str!=='string'){return str;}
args=[];for(l=0,len1=arguments.length;l<len1;l++){arg=arguments[l];args.push(arg);}
ref=args.slice(1);for(m=0,len2=ref.length;m<len2;m++){arg=ref[m];if(arg===void 0){arg='';}
str=str.replace("%s",arg);}
return str;};}
if(typeof angular.self_translate!=='function'){angular.self_translate=function(obj){var key;if(typeof obj!=='object'){return obj;}
key=Object.keys(obj)[0];return obj[key];};}
console.apiError=function(error,error_stack){var k,v;console.info('---------------API Error--------------');for(k in error){v=error[k];if(v){console.error(k,':',v);}}
if(error_stack){console.info(error_stack);}
