...
return console.info('---------------------------');};assertParam=function(param,type,failback){var result;if(!failback){failback='';}
if(!type){type='string';}
...
resetCurrentMedia();historyManager.record();return dispatchSAWEvent();};execInsertLink=function(linkObj){var a,class_,class_str,href,link,target,target_str;if(!savedRange||!linkObj){return;}
...
if(class_){class_str='class="'+class_+'"';}else{class_str='';}
link='<a '+class_str+' '+target_str+' href="'+href+'">'+href+'</a>';pasteHtmlAtCaret(link);}
...
return dict;};angular.str2opts=function(opts_str,separator){var _key,e,l,len,op,opts,opts_list;if(typeof opts_str!=='string'){return false;}
try{opts_list=str.split(separator||',');}catch(error1){e=error1;opts_list=[];}
opts={};for(l=0,len=opts_list.length;l<len;l++){op=opts_list[l];_key=op.trim();if(_key){opts[op]=true;}}
