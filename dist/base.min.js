...
i+=1;}
return _len;};angular.pop=function(obj,key){var value;value=obj[key];delete obj[key];return value;};angular.set=function(list){var i,l,len,newlist;newlist=[];for(l=0,len=list.length;l<len;l++){i=list[l];if(indexOf.call(newlist,i)<0){newlist.push(i);}}
...
return str.substr(str.length-text.length)===text;};angular.currency=function(amount,symbol,fractionSize){var size;if(symbol===void 0){symbol='';}
...
return date;};angular.sortedby=function(source,sort_keys,reverse){var compare,key,l,len,new_list,parse_sorts,sorts;sorts=[];if(!angular.isArray(sort_keys)){sort_keys=[sort_keys];}
...
return sorts.push([key,direction]);}};for(l=0,len=sort_keys.length;l<len;l++){key=sort_keys[l];parse_sorts(key);}
compare=function(a,b){var direction,len1,m,sort;for(m=0,len1=sorts.length;m<len1;m++){sort=sorts[m];key=sort[0];direction=sort[1];if(a[key]<b[key]){return-1*direction;}
...
return dict;};angular.str2opts=function(opts_str,separator,exclude_mark){var _key,e,l,len,op,op_key,opts,opts_list;if(typeof opts_str!=='string'){return false;}
...
opts={};for(l=0,len=opts_list.length;l<len;l++){op=opts_list[l];_key=op.trim();op_key=_key.replace(exclude_mark,'');if(_key){opts[op_key]=Boolean(_key.indexOf(exclude_mark)!==0);}}
return opts;};angular.list2dict=function(list,key,key2){var dict,idx,item,l,len;if(typeof key!=='string'){key=null;}
if(typeof key2!=='string'){key2=null;}
dict={};for(idx=l=0,len=list.length;l<len;idx=++l){item=list[idx];if(!key){key=idx.toString();}
dict[item[key]]=key2?item[key2]:item;}
return dict;};angular.str2list=function(str,is_set,separator,whitespace){var e,l,len,newlist,orglist,s;if(angular.isArray(str)){return str;}else if(typeof str!=='string'){return[];}
if(!separator){separator=',';str=str.replace(/[，]/g,',');}
try{orglist=str.split(separator);}catch(error1){e=error1;orglist=[];}
newlist=[];for(l=0,len=orglist.length;l<len;l++){s=orglist[l];if(s){if(!whitespace){s=s.trim();}
...
return str;};angular.list_index=function(list,item,attr){var equals,idx,l,len,obj;equals=function(obj1,obj2,attr){if(attr){return angular.equals(obj1[attr],obj2[attr]);}else{return angular.equals(obj1,obj2);}};for(idx=l=0,len=list.length;l<len;idx=++l){obj=list[idx];if(equals(obj,item,attr)){return idx;}}
...
return list;};angular.list_pop=function(list,item,attr){var equals,idx,l,len,obj,pop_obj;equals=function(obj1,obj2,attr){if(attr){return angular.equals(obj1[attr],obj2[attr]);}else{return angular.equals(obj1,obj2);}};pop_obj=null;for(idx=l=0,len=list.length;l<len;idx=++l){obj=list[idx];if(equals(obj,item,attr)){pop_obj=obj;list.splice(idx,1);break;}}
return pop_obj;};angular.list_remove=function(list,item,attr){var equals,i,idx,indexes,l,len,len1,m,obj;equals=function(obj1,obj2,attr){if(attr){return angular.equals(obj1[attr],obj2[attr]);}else{return angular.equals(obj1,obj2);}};indexes=[];for(idx=l=0,len=list.length;l<len;idx=++l){obj=list[idx];if(equals(obj,item,attr)){indexes.unshift(idx);}}
for(m=0,len1=indexes.length;m<len1;m++){i=indexes[m];list.splice(i,1);}
return list;};angular.update_dict=function(obj,obj2){var k,v;for(k in obj2){v=obj2[k];obj[k]=v;}
return obj;}; angular.format_tags=function(tags,limit,upper){var _tag,_tags,l,len,styl,tag;if(!angular.isArray(tags)){return[];}
if(typeof limit!=='number'||!limit){limit=60;}
styl=function(text){if(upper){return angular.upper(text);}else{return angular.lower(text);}};_tags=[];for(l=0,len=tags.length;l<len;l++){tag=tags[l];if(typeof tag!=='string'){continue;}
...
regex=/^[\w]+:/i;return Boolean(str.match(regex));};angular.addURLParams=function(url,params){var _add,item,k,l,len,v;if(!angular.isObject(params)){return url;}
_add=function(url,key,value){var joint;joint=url.indexOf('?')>-1?'&':'?';key=encodeURIComponent(key);value=encodeURIComponent(value);url=url+joint+key+'='+value;return url;};for(k in params){v=params[k];if(angular.isArray(v)){for(l=0,len=v.length;l<len;l++){item=v[l];url=_add(url,k,item);}}else{url=_add(url,k,v);}}
return url;};angular.getURLParams=function(url,unique){var hash_idx,k,l,len,pair,params,query_idx,query_vars,v;if(typeof url!=='string'){return{};}
if(unique===void 0){unique=true;}
hash_idx=url.indexOf('#');if(hash_idx>-1){url=url.substring(0,hash_idx);}
params={};query_idx=url.indexOf('?');if(query_idx>-1){query_vars=url.split('?')[1].split('&');for(l=0,len=query_vars.length;l<len;l++){v=query_vars[l];pair=v.split('=');k=pair[0];v=pair[1];if(!k){continue;}else if(!params[k]){params[k]=v;}else if(!unique){if(!angular.isArray(params[k])){params[k]=[params[k]];}
params[k].push(v);}}}
