...
$$CookieWriter.$inject=['$document','$log','$browser'];angular.module('ngCookies').provider('$$cookieWriter',function $$CookieWriterProvider(){this.$get=$$CookieWriter;});})(window,window.angular);(function(){var supWysiwygEditor,indexOf=[].indexOf;supWysiwygEditor=function(){var $btn_elements_stack,$btn_indent,$btn_outdent,$currentAudio,$currentImage,$currentLink,$currentPreset,$currentVideo,$editor,$history,$history_step,$options,$property,$toolbar,BFBL,BTN_LIST,STYLE_TAGS,baseEventHanlder,blowupHtml,blurHandler,blurHook,childOfElement,cleanStyles,clearEmptyElements,clearFormat,closeHTMLHook,destroySpan,destroy_editor,dispatchSAWEvent,doInsertAudio,doInsertImg,doInsertVideo,doReplaceAudio,doReplaceImg,doReplaceVideo,doubleClickHandler,editLinkHook,editMediaHook,editPresetHook,endEl,errlog,evtManager,execCmd,execFormatBlock,execInsertLink,execInsertMedia,execInsertPreset,execInsertScript,execRemoveLink,execRemoveMedia,execRemovePreset,execReplaceLink,execReplaceMedia,execReplacePreset,expose,findElement,findSibling,findStylesParent,findTopElement,focusHandler,focusHook,getAllElementsFromSelection,getCurrentSelection,getDisplayType,getSameParent,getSelectionHtml,historyManager,inEditor,indentText,init_editor,insertA,insertHR,insertLinkHook,insertMedia,insertMediaHook,insertOrderedList,insertPreset,insertPresetHook,insertScript,insertScriptHook,insertUnorderedList,isFamily,isHTMLElement,justifyCenter,justifyFull,justifyLeft,justifyRight,load_options,log,makeBold,makeCode,makeDiv,makeFontBig,makeFontSmall,makeH1,makeH2,makeH3,makeH4,makeH5,makeH6,makeItalic,makeMark,makeP,makePre,makeQuote,makeStrike,makeSub,makeSup,makeUnderline,now,openHTMLHook,outdentText,pasteHandler,pasteHtmlAtCaret,placeCaretAfterNode,reDo,removeInlineStyle,removeTag,resetCurrentLink,resetCurrentMedia,resetCurrentPreset,restoreSelection,richEditorModel,saveSelection,savedRange,selObj,selRange,selTextHook,selectHandler,selectedNode,selectedOrigin,setKeyboard,setRange,setRangeByStartEnd,set_blur_hook,set_edit_link_hook,set_edit_media_hook,set_edit_preset_hook,set_focus_hook,set_html_on_close_hook,set_html_on_open_hook,set_insert_link_hook,set_insert_media_hook,set_insert_preset_hook,set_insert_script_hook,set_select_text_hook,set_toolbar,smartFormatBlock,startEl,switchIndentOutdentBtns,targetA,targetAudio,targetImg,targetPreset,targetText,targetVideo,testBlackList,toggleHtml,toggleTextWrapper,unDo,unset_toolbar,ver,watchDoubleClick,watchSelected;ver='1.8.0';now=Date.now();$options={history:100};$property={id:'',order:0};$editor=void 0;$toolbar=void 0;selObj=void 0;selRange=void 0;selectedNode=void 0;selectedOrigin=void 0;startEl=void 0;endEl=void 0;savedRange=void 0;richEditorModel=true;focusHook=void 0;blurHook=void 0;selTextHook=void 0;insertLinkHook=void 0;editLinkHook=void 0;insertMediaHook=void 0;editMediaHook=void 0;insertPresetHook=void 0;editPresetHook=void 0;insertScriptHook=void 0;openHTMLHook=void 0;closeHTMLHook=void 0;$currentImage={element:void 0,link:void 0};$currentVideo={element:void 0};$currentAudio={element:void 0};$currentLink={element:void 0};$currentPreset={element:void 0};$history=[];$history_step=null;$btn_indent=void 0;$btn_outdent=void 0;$btn_elements_stack={};BTN_LIST=[{name:'h1',handler:'makeH1'},{name:'h2',handler:'makeH2'},{name:'h3',handler:'makeH3'},{name:'h4',handler:'makeH4'},{name:'h5',handler:'makeH5'},{name:'h6',handler:'makeH6'},{name:'div',handler:'makeDiv'},{name:'p',handler:'makeP'},{name:'blockquote',handler:'makeQuote'},{name:'pre',handler:'makePre'},{name:'code',handler:'makeCode'},{name:'mark',handler:'makeMark'},{name:'i',handler:'makeItalic'},{name:'b',handler:'makeBold'},{name:'strike',handler:'makeStrike'},{name:'underline',handler:'makeUnderline'},{name:'small',handler:'makeFontSmall'},{name:'big',handler:'makeFontBig'},{name:'sub',handler:'makeSub'},{name:'sup',handler:'makeSup'},{name:'ol',handler:'insertOrderedList'},{name:'ul',handler:'insertUnorderedList'},{name:'outdent',handler:'outdentText'},{name:'indent',handler:'indentText'},{name:'hr',handler:'insertHR'},{name:'img',handler:'insertMedia'},{name:'preset',handler:'insertPreset'},{name:'a',handler:'insertA'},{name:'jleft',handler:'justifyLeft'},{name:'jcenter',handler:'justifyCenter'},{name:'jright',handler:'justifyRight'},{name:'jfull',handler:'justifyFull'},{name:'redo',handler:'reDo'},{name:'undo',handler:'unDo'},{name:'clear',handler:'clearFormat'},{name:'cleanup',handler:'cleanStyles'},{name:'script',handler:'insertScript'},{name:'html',handler:'toggleHtml'}];BFBL=['OL','UL','LI','TD','TH','TR','TBODY','TABLE','THEAD','TFOOT','DL','DD','DT'];STYLE_TAGS=['b','i','u','s','mark','strong','sup','sub','small','big','strike','del','font','span'];selectHandler=function(e){getCurrentSelection();watchSelected(e);};doubleClickHandler=function(e){getCurrentSelection();watchDoubleClick(e);};focusHandler=function(e){getCurrentSelection();savedRange=saveSelection();if(!childOfElement(e.target,$toolbar,true)){if(typeof focusHook==='function'){focusHook();}}};blurHandler=function(e){if(inEditor(e.target,true)){return;}
if(!childOfElement(e.target,$toolbar,true)){if(typeof blurHook==='function'){blurHook();}}};pasteHandler=function(e){var text;if(e.clipboardData&&e.clipboardData.getData){e.preventDefault();text=e.clipboardData.getData("text/plain");return document.execCommand("insertText",false,text);}else if(window.clipboardData&&window.clipboardData.getData){e.preventDefault();text=window.clipboardData.getData("Text");return document.execCommand("insertText",false,text);}};targetText=function(text){var ref,text_types;text_types=['h1','h2','h3','h4','h5','h6','p','blockquote','pre','code','div'];if(!text||!inEditor(text)){text=selectedNode;}
if(!text||(ref=text.tagName.toLowerCase(),indexOf.call(text_types,ref)<0)){return;}
if(typeof selTextHook==='function'){return selTextHook(text);}};targetImg=function(img){var imgA,imgObj,new_src,src;if(typeof editMediaHook==='function'){$currentImage.element=img;imgObj={type:'image',src:img.getAttribute('src'),title:img.getAttribute('alt'),width:img.getAttribute('width'),height:img.getAttribute('height'),alt:img.getAttribute('alt'),class:img.getAttribute('class')};imgA=img.parentElement;if(imgA&&inEditor(imgA)&&imgA.tagName==='A'){$currentImage.link=imgA;imgObj.link=imgA.getAttribute('href');imgObj.target=imgA.getAttribute('target');imgObj.class=imgA.getAttribute('class');}else{$currentImage.link=null;}
return editMediaHook(imgObj);}else{src=img.getAttribute('src');new_src=prompt('Img:',src);if(new_src){img.setAttribute('src',new_src);}else if(new_src!==null){img.parentElement.removeChild(img);}
return historyManager.record();}};targetVideo=function(video){var new_src,src,videoObj,video_source;if(typeof editMediaHook==='function'){$currentVideo.element=video;try{video_source=video.querySelector('source').getAttribute('src');}catch(error){video_source='';}
videoObj={type:'video',src:video.getAttribute('src')||video_source,width:video.getAttribute('width'),height:video.getAttribute('height'),poster:video.getAttribute('poster'),controls:video.hasAttribute('controls'),loop:video.hasAttribute('loop'),autoplay:video.hasAttribute('autoplay'),class:video.getAttribute('class')};return editMediaHook(videoObj);}else{src=video.getAttribute('src');new_src=prompt('Video:',src);if(new_src){video.setAttribute('src',new_src);video.setAttribute('controls',true);}else if(new_src!==null){video.parentElement.removeChild(video);}
return historyManager.record();}};targetAudio=function(audio){var audioObj,new_src,src;if(typeof editMediaHook==='function'){$currentAudio.element=audio;audioObj={type:'audio',src:audio.getAttribute('src'),loop:audio.hasAttribute('loop'),autoplay:audio.hasAttribute('autoplay'),controls:audio.hasAttribute('controls'),class:audio.getAttribute('class')};return editMediaHook(audioObj);}else{src=audio.getAttribute('src');new_src=prompt('Audio:',src);if(new_src){audio.setAttribute('src',new_src);audio.setAttribute('controls',true);}else if(new_src!==null){audio.parentElement.removeChild(audio);}
return historyManager.record();}};targetA=function(a){var linkObj,new_url;if(typeof editLinkHook==='function'){$currentLink.element=a;linkObj={href:a.getAttribute('href'),target:a.getAttribute('target'),class:a.getAttribute('class')};return editLinkHook(linkObj);}else{new_url=prompt('Link:',url);if(new_url){a.setAttribute('href',new_url);}else if(new_url!==null){a.outerHTML=a.innerHTML;}
return historyManager.record();}};targetPreset=function(div){var presetObj,preset_code,preset_type;if(typeof editPresetHook==='function'){preset_type=div.getAttribute('preset');if(preset_type){try{preset_code=decodeURIComponent(div.getAttribute('code')||'');}catch(error){preset_code='';}
presetObj={type:preset_type,code:preset_code,rendered:div.innerHTML};$currentPreset.element=div;return editPresetHook(presetObj);}else{return resetCurrentPreset();}}};insertOrderedList=function(e){var content,node,ol;if(!inEditor(selectedNode,true)){return;}
...
switchIndentOutdentBtns();historyManager.record();return getCurrentSelection();};insertUnorderedList=function(e){var content,node,ul;if(!inEditor(selectedNode,true)){return;}
...
switchIndentOutdentBtns();historyManager.record();return getCurrentSelection();};outdentText=function(e){if(findElement(selectedNode,'LI')||findElement(selectedNode,'OL')){execCmd('outdent');return switchIndentOutdentBtns();}};indentText=function(e){if(findElement(selectedNode,'LI')||findElement(selectedNode,'OL')){execCmd('indent');return switchIndentOutdentBtns();}};makeDiv=function(e){smartFormatBlock('div');return targetText();};makeH1=function(e){smartFormatBlock('h1');return targetText();};makeH2=function(e){smartFormatBlock('h2');return targetText();};makeH3=function(e){smartFormatBlock('h3');return targetText();};makeH4=function(e){smartFormatBlock('h4');return targetText();};makeH5=function(e){smartFormatBlock('h5');return targetText();};makeH6=function(e){smartFormatBlock('h6');return targetText();};makePre=function(e){smartFormatBlock('pre');return targetText();};makeQuote=function(e){smartFormatBlock('blockquote');return targetText();};makeP=function(e){smartFormatBlock('p');return targetText();};makeBold=function(e){return execCmd('bold');};makeItalic=function(e){return execCmd('italic');};justifyLeft=function(e){return execCmd('justifyLeft');};justifyRight=function(e){return execCmd('justifyRight');};justifyCenter=function(e){return execCmd('justifyCenter');};justifyFull=function(e){return execCmd('justifyFull');};makeStrike=function(e){return execCmd('strikeThrough');};makeUnderline=function(e){return execCmd('underline');};makeSup=function(e){return execCmd('superscript');};makeSub=function(e){return execCmd('subscript');};reDo=function(e){return historyManager.redo();};unDo=function(e){return historyManager.undo();};makeFontSmall=function(e){if(!inEditor(selectedNode,true)){return;}
if(toggleTextWrapper('small')){clearEmptyElements(selectedNode,['big','small']);getCurrentSelection();return historyManager.record();}};makeFontBig=function(e){if(!inEditor(selectedNode,true)){return;}
if(toggleTextWrapper('big')){clearEmptyElements(selectedNode,['big','small']);getCurrentSelection();return historyManager.record();}};makeMark=function(e){if(!inEditor(selectedNode,true)){return;}
if(toggleTextWrapper('mark')){clearEmptyElements(selectedNode,['mark']);getCurrentSelection();return historyManager.record();}};makeCode=function(e){var element;if(!inEditor(selectedNode)){return;}
element=findElement(selectedNode,'CODE');if(element){element.outerHTML='<div>'+element.innerHTML+'</div>';}else{element=testBlackList(selectedNode);element.outerHTML='<code>'+element.innerHTML+'</code>';}
selObj.extend(element,0);return historyManager.record();};clearFormat=function(e){var content,element,i,j,len1,sel_elements;if(!inEditor(selectedNode,true)){return;}
sel_elements=getAllElementsFromSelection();for(j=0,len1=sel_elements.length;j<len1;j++){element=sel_elements[j];element=findStylesParent(element);element.removeAttribute('style');if(!element.innerHTML||!inEditor(element)){continue;}
content=element.innerHTML;i=0;while(i<STYLE_TAGS.length){content=removeTag(content,STYLE_TAGS[i],true);i+=1;}
element.innerHTML=removeInlineStyle(content);}
historyManager.record();return getCurrentSelection();};cleanStyles=function(e){var el,els,j,k,len1,len2,span,span_els;if(!$editor.innerHTML){return;}
els=$editor.querySelectorAll('[style]');for(j=0,len1=els.length;j<len1;j++){el=els[j];el.removeAttribute('style');}
span_els=$editor.querySelectorAll('span');for(k=0,len2=span_els.length;k<len2;k++){span=span_els[k];span.outerHTML=span.innerHTML;}
historyManager.record();return getCurrentSelection();};blowupHtml=function(e){var current,end,i,resultObj,room,start,tmp_trigger,trigger;if(inEditor(selectedNode)){start=startEl.parentElement;end=endEl.parentElement;if(start!==end){resultObj=getSameParent(start,end);room=resultObj.point.childNodes;i=0;while(i<room.length){if(room[i].nodeType===1){tmp_trigger=false;if(room[i]===resultObj.start||room[i]===resultObj.end){if(trigger){tmp_trigger=false;}else{trigger=true;tmp_trigger=trigger;}}
if(trigger){room[i].outerHTML='<div>'+room[i].innerText+'</dvi>';}
trigger=tmp_trigger;}
i+=1;}}else{current=testBlackList(selectedNode);if(current){current=current.outerHTML='<div>'+current.innerText+'</div>';}else{selectedNode.outerHTML='<div>'+selectedNode.innerText+'</div>';}}}else if(inEditor(selectedNode,true)){$editor.innerHTML=$editor.innerText;}
return historyManager.record();};insertHR=function(e){return execCmd('insertHorizontalRule');};insertA=function(e){var link;if(inEditor(selectedNode,true)){if(typeof insertLinkHook==='function'){savedRange=saveSelection();insertLinkHook(savedRange.endOffset>0);}else{link=prompt('Link:','#');execCmd('createLink',false,link);}
return historyManager.record();}};insertMedia=function(e){var src;if(inEditor(selectedNode,true)){if(typeof insertMediaHook==='function'){savedRange=saveSelection();insertMediaHook();}else{src=prompt('Img:','http://');if(src!==''&&src!=='http://'){execCmd('insertImage',false,src);}}
return historyManager.record();}};insertPreset=function(e){var preset;if(inEditor(selectedNode,true)){if(typeof insertPresetHook==='function'){savedRange=saveSelection();insertPresetHook();}else{savedRange=saveSelection();preset=prompt('Preset:','');if(preset!==''){execInsertPreset(preset);}}
return historyManager.record();}};insertScript=function(e){var script;if(inEditor(selectedNode,true)){if(typeof insertScriptHook==='function'){savedRange=saveSelection();insertScriptHook();}else{savedRange=saveSelection();script=prompt('Script:','');if(script!==''){execInsertScript(script);}}
return historyManager.record();}};toggleHtml=function(e){if(inEditor(selectedNode,true)&&typeof openHTMLHook==='function'){historyManager.record();savedRange=saveSelection();return openHTMLHook();}};getCurrentSelection=function(){var node;node=void 0;selObj=window.getSelection();if(selObj.type!=='None'&&selObj.rangeCount>0){selRange=selObj.getRangeAt(0);startEl=selRange.startContainer;endEl=selRange.endContainer;node=selObj.anchorNode;selectedOrigin=node;while(node.nodeType!==1){node=node.parentNode;}
...
return Boolean(find);};watchSelected=function(e){var target;switchIndentOutdentBtns();target=findStylesParent(e.target||selectedNode);return targetText(target);};watchDoubleClick=function(e){var child_img,preset_target,tagname,target;if(selectedNode&&selectedNode.tagName==='A'){target=selectedNode;}else{target=findStylesParent(e.target||selectedNode);}
preset_target=findElement(target,'DIV','preset');if(preset_target){target=preset_target;}
tagname=target.hasAttribute('preset')?'preset':target.tagName.toLowerCase();switch(tagname){case'preset':return targetPreset(target);case'img':return targetImg(target);case'video':return targetVideo(target);case'audio':return targetAudio(target);case'a':child_img=target.querySelector('img');if(child_img){return targetImg(child_img);}else{return targetA(target);}}};switchIndentOutdentBtns=function(){getCurrentSelection();if(findElement(selectedNode,'LI')||findElement(selectedNode,'OL')){if($btn_indent){$btn_indent.removeAttribute('disabled');}
if($btn_outdent){$btn_outdent.removeAttribute('disabled');}}else{if($btn_indent){$btn_indent.setAttribute('disabled',true);}
if($btn_outdent){$btn_outdent.setAttribute('disabled',true);}}};saveSelection=function(){var sel;if(window.getSelection){sel=window.getSelection();if(sel.getRangeAt&&sel.rangeCount){return sel.getRangeAt(0);}}else if(document.selection&&document.selection.createRange){return document.selection.createRange();}
...
if(element.parentElement){elementTagName=element.parentElement.tagName;if(ref=elementTagName.toLowerCase(),indexOf.call(STYLE_TAGS,ref)>=0){element=findStylesParent(element.parentElement);}else{element=element.parentElement;}}
...
return findElement(element.parentElement,tag,attr);};findTopElement=function(element){if(element.parentElement===$editor&&element.nodeType===1){return element;}else if(inEditor(element.parentElement)){return findTopElement(element.parentElement);}
return null;};clearEmptyElements=function(element,tags){var el,empty_els,j,len1,results,tag;if(!element||!inEditor(element,true)){return;}
element=element===$editor?element:element.parentElement;results=[];for(j=0,len1=tags.length;j<len1;j++){tag=tags[j];empty_els=element.querySelectorAll(tag);results.push((function(){var k,len2,results1;results1=[];for(k=0,len2=empty_els.length;k<len2;k++){el=empty_els[k];if(!el.innerHTML){results1.push(el.parentElement.removeChild(el));}else{results1.push(void 0);}}
return results1;})());}
return results;};toggleTextWrapper=function(target){var content,el,regex;if(!selectedNode){console.error("Can not wrapper while no element been selected.");return false;}
target=target.toLowerCase();regex=new RegExp('<[\/]?'+target+'.*?>','ig');el=document.createElement(target);el.appendChild(selRange.cloneContents());if(el.querySelector(target)){content=el.innerHTML.replace(regex,'');selRange.deleteContents();pasteHtmlAtCaret(content,true);}else{el.innerHTML=el.innerHTML.replace(regex,'');if(!el.innerHTML){return false;}
selRange.deleteContents();selRange.insertNode(el);setRange(el);}
return true;};getDisplayType=function(element){var style;style=element.currentStyle||window.getComputedStyle(element,"");return style.display;};pasteHtmlAtCaret=function(html,selectPastedContent){var el,firstNode,frag,lastNode,node,originalRange,range,sel;if(window.getSelection){sel=window.getSelection();if(sel.getRangeAt&&sel.rangeCount>0){range=sel.getRangeAt(0);range.deleteContents();el=document.createElement('div');el.innerHTML=html;frag=document.createDocumentFragment();node=void 0;lastNode=void 0;while(node=el.firstChild){lastNode=frag.appendChild(node);}
...
return str;};removeTag=function(str,tag,force){var regex1,regex2;if(typeof tag!=='string'){return;}
tag=tag.toLowerCase();regex1=new RegExp('<\s*'+tag+'(?:\s+)*?[^>]*?>(.*?)</\s*'+tag+'\s*>','ig');while(regex1.test(str)){str=str.replace(regex1,'$1');}
if(force){regex2=new RegExp('(<'+tag+'(?:\s+)*?[^>]*?>)|(</\s*'+tag+'\s*>)','ig');while(regex2.test(str)){str=str.replace(regex2,'');}}
return str;};placeCaretAfterNode=function(node){var range,selection;if(typeof window.getSelection==='undefined'||!node.parentNode){return;}
...
results.push(i+=1);}
return results;};setKeyboard=function(editor){var key;key=[];key['shift']=false;key['enter']=false;editor.addEventListener('keydown',function(e){if(e.keyCode===16){key['shift']=true;}
if(e.keyCode===8||e.keyCode===46||e.keyCode===17||e.keyCode===91||e.keyCode===88){historyManager.record();}
if(e.keyCode===13){key['enter']=true;historyManager.record();}
if(key['shift']&&key['enter']){e.preventDefault();return pasteHtmlAtCaret('<br>');}});editor.addEventListener('keyup',function(e){if(e.keyCode===16){key['shift']=false;}
if(e.keyCode===13){key['enter']=false;}});};getSelectionHtml=function(strip){var container,html,i,len,sel;html='';if(typeof window.getSelection!=='undefined'){sel=window.getSelection();if(!strip){if(sel.rangeCount){container=document.createElement('div');i=0;len=sel.rangeCount;while(i<len){container.appendChild(sel.getRangeAt(i).cloneContents());i+=1;}
...
event=new Event('saw');return $editor.dispatchEvent(event);};baseEventHanlder=function(handler){return dispatchSAWEvent();};evtManager={register:function(target,handler,event){if(typeof event!=='string'){event='click';}
if(typeof target==='object'){target.addEventListener(event,handler);target.addEventListener(event,baseEventHanlder);}},remove:function(target,handler,event){if(typeof event!=='string'){event='click';}
if(typeof target==='object'){if(typeof handler==='function'){target.removeEventListener(event,handler);target.removeEventListener(event,baseEventHanlder);}}}};resetCurrentMedia=function(){$currentImage={link:void 0,element:void 0};$currentVideo={element:void 0};return $currentAudio={element:void 0};};resetCurrentLink=function(){return $currentLink={element:void 0};};resetCurrentPreset=function(){return $currentPreset={element:void 0};};doInsertImg=function(imgObj){var alt,class_,class_str,height,href,html,md_str,size_str,src,target,target_str,width;src=imgObj.src;alt=imgObj.title;href=imgObj.href;target=imgObj.target;width=imgObj.width;height=imgObj.height;class_=imgObj.class;if(class_){class_str='class="'+class_+'"';}else{class_str='';}
...
href=linkObj.href;target=linkObj.target;class_=linkObj.class;restoreSelection(savedRange);if(savedRange.endOffset>0){execCmd('createLink',false,href);getCurrentSelection();a=selectedNode;if(a.tagName==='A'){if(target){a.setAttribute('target',target);}
if(class_){a.setAttribute('class',class_);}}}else{if(target){target_str='target="'+target+'"';}else{target_str='';}
if(class_){class_str='class="'+class_+'"';}else{class_str='';}
name=linkObj.name||href;link='<a '+class_str+' '+target_str+' href="'+href+'">'+name+'</a>';pasteHtmlAtCaret(link);}
...
a.outerHTML=a.innerHTML;resetCurrentLink();historyManager.record();return dispatchSAWEvent();};execInsertPreset=function(preset){var element,preset_element;if(!savedRange||!preset){return;}
preset_element=document.createElement('div');if(preset.type){preset_element.setAttribute('preset',preset.type);}
if(preset.code){preset_element.setAttribute('code',encodeURIComponent(preset.code));}
preset_element.innerHTML=preset.rendered||'';$editor.appendChild(preset_element);restoreSelection(savedRange);element=findTopElement(selectedNode);if(element&&element.nextSibling){$editor.insertBefore(preset_element,element.nextSibling);}else{$editor.appendChild(preset_element);}
...
if(preset.type){preset_element.setAttribute('preset',preset.type);}else{preset_element.removeAttribute('preset');}
if(preset.code){preset_element.setAttribute('code',encodeURIComponent(preset.code));}else{preset_element.removeAttribute('code');}
...
div.removeAttribute('preset');div.removeAttribute('key');div.innerHTML='&nbsp;';resetCurrentPreset();historyManager.record();return dispatchSAWEvent();};execInsertScript=function(script){if(!savedRange||!script){return;}
...
$editor.addEventListener('keyup',selectHandler);window.addEventListener('mousedown',blurHandler);set_toolbar();setKeyboard($editor);historyManager.record();console.info('supAngularWysiwyg('+ver+'): '+$property.name+' inited');};destroy_editor=function(){$editor.removeEventListener('mousedown',focusHandler);$editor.removeEventListener('click',selectHandler);$editor.removeEventListener('dbclick',selectHandler);$editor.removeEventListener('paste',pasteHandler);if($toolbar){$toolbar.removeEventListener('mousedown',focusHandler);}
$editor.removeEventListener('keyup',selectHandler);window.removeEventListener('mousedown',blurHandler);unset_toolbar();console.info('supAngularWysiwyg: destroyed');};set_toolbar=function(){var btn,btn_el,j,len1,results;if(!$toolbar){return;}
results=[];for(j=0,len1=BTN_LIST.length;j<len1;j++){btn=BTN_LIST[j];btn_el=$toolbar.querySelector('.btn-'+btn.name);if(btn_el){if(btn.name==='indent'){$btn_indent=btn_el;}
if(btn.name==='outdent'){$btn_outdent=btn_el;}
evtManager.register(btn_el,eval(btn.handler));results.push($btn_elements_stack[btn.name]=btn_el);}else{results.push(void 0);}}
return results;};unset_toolbar=function(){var btn,btn_el,j,len1,results;if(!$toolbar){return;}
results=[];for(j=0,len1=BTN_LIST.length;j<len1;j++){btn=BTN_LIST[j];btn_el=$btn_elements_stack[btn.name];if(btn_el){results.push(evtManager.remove(btn_el,eval(btn.handler)));}else{results.push(void 0);}}
return results;};load_options=function(opt){var key,results;if(typeof opt!=='object'){return;}
results=[];for(key in $options){if(key in opt){results.push($options[key]=opt[key]);}else{results.push(void 0);}}
return results;};set_insert_link_hook=function(func){if(typeof func==='function'){return insertLinkHook=func;}else{return false;}};set_edit_link_hook=function(func){if(typeof func==='function'){return editLinkHook=func;}else{return false;}};set_edit_media_hook=function(func){if(typeof func==='function'){return editMediaHook=func;}else{return false;}};set_insert_media_hook=function(func){if(typeof func==='function'){return insertMediaHook=func;}else{return false;}};set_insert_preset_hook=function(func){if(typeof func==='function'){return insertPresetHook=func;}else{return false;}};set_edit_preset_hook=function(func){if(typeof func==='function'){return editPresetHook=func;}else{return false;}};set_insert_script_hook=function(func){if(typeof func==='function'){return insertScriptHook=func;}else{return false;}};set_select_text_hook=function(func){if(typeof func==='function'){return selTextHook=func;}else{return false;}};set_focus_hook=function(func){if(typeof func==='function'){return focusHook=func;}else{return false;}};set_blur_hook=function(func){if(typeof func==='function'){return blurHook=func;}else{return false;}};set_html_on_open_hook=function(func){if(typeof func==='function'){return openHTMLHook=func;}else{return false;}};set_html_on_close_hook=function(func){if(typeof func==='function'){return closeHTMLHook=func;}else{return false;}};expose={init:init_editor,destroy:destroy_editor,hooks:{insert_link:set_insert_link_hook,edit_link:set_edit_link_hook,insert_media:set_insert_media_hook,edit_media:set_edit_media_hook,insert_preset:set_insert_preset_hook,edit_preset:set_edit_preset_hook,insert_script:set_insert_script_hook,select_text:set_select_text_hook,focus:set_focus_hook,blur:set_blur_hook,html_on_open:set_html_on_open_hook,html_on_close:set_html_on_close_hook},insertMedia:execInsertMedia,replaceMedia:execReplaceMedia,removeMedia:execRemoveMedia,insertLink:execInsertLink,replaceLink:execReplaceLink,removeLink:execRemoveLink,insertPreset:execInsertPreset,replacePreset:execReplacePreset,removePreset:execRemovePreset,insertScript:execInsertScript};Object.defineProperty(expose,'version',{get:function(){return ver;}});Object.defineProperty(expose,'id',{get:function(){return $property.id;}});Object.defineProperty(expose,'order',{get:function(){return $property.order;}});Object.defineProperty(expose,'element',{get:function(){return $editor;}});Object.defineProperty(expose,'options',{get:function(){return $options;}});return expose;};angular.module('supAngularWysiwyg',[]).directive('supAngularWysiwyg',['$rootScope','$window','$interpolate','$timeout',function($rootScope,$window,$interpolate,$timeout){return{restrict:'EA',require:'?ngModel',scope:{toolbar:'@',history:'@',default:'@',embedMode:'@',readonly:'@',presets:'='},link:function(scope,element,attrs,ngModel){var default_data,dirty,e,editor,j,len1,options,p,presets,readViewText,ref,root,toolbar;if(!ngModel){return;}
...
$timeout(function(){var loaded;if(scope.readonly){editor='readonly';}else{editor.init(element[0],toolbar,options);}
loaded={label:attrs.label||attrs.name,model:attrs.ngModel};$rootScope.$emit('saw.loaded',loaded,editor);if(root!==$rootScope){return root.$emit('saw.loaded',loaded,editor);}},0);editor.hooks.select_text(function(el){return root.$emit('saw.select_text',editor,el);});editor.hooks.insert_link(function(has_selection){return root.$emit('saw.insert_link',editor,has_selection);});editor.hooks.edit_link(function(link){return root.$emit('saw.edit_link',editor,link);});editor.hooks.insert_media(function(){return root.$emit('saw.insert_media',editor);});editor.hooks.edit_media(function(media){return root.$emit('saw.edit_media',editor,media);});editor.hooks.insert_preset(function(){return root.$emit('saw.insert_preset',editor,presets);});editor.hooks.edit_preset(function(curr_preset){return root.$emit('saw.edit_preset',editor,presets,curr_preset);});editor.hooks.insert_script(function(){return root.$emit('saw.insert_script',editor);});editor.hooks.focus(function(){return root.$emit('saw.focus',editor);});editor.hooks.blur(function(){return root.$emit('saw.blur',editor);});editor.hooks.html_on_open(function(){return root.$emit('saw.open_html',editor,ngModel.$viewValue);});editor.hooks.html_on_close(function(){return root.$emit('saw.close_html',editor,ngModel.$viewValue);});dirty=false;if(scope.default){default_data=scope.default;}else{default_data=$interpolate(element.html())($rootScope);}
...
return opts;};angular.list2dict=function(list,key,key2){var dict,idx,item,l,len1;if(typeof key!=='string'){key=null;}
if(typeof key2!=='string'){key2=null;}
dict={};for(idx=l=0,len1=list.length;l<len1;idx=++l){item=list[idx];if(!key){key=idx.toString();}
