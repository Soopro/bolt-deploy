...
root=$rootScope;if(!scope.embedMode&&parent.document!==document){root=parent.angular.element(parent.document).scope();toolbar=parent.document.querySelector('[id='+toolbar+']');}
...
readViewText();e.stopPropagation();});return scope.$on('$destroy',function(){return editor.destroy();});}};}]);}).call(this);(function(){var is_exports,root,supLocales;is_exports=typeof exports!=="undefined"&&exports!==null;root=is_exports?exports:this;if(!root.sup){root.sup={};}
sup.localizedDict={};supLocales=function(opts){var _load_dict,_trans_key,append,i,insert,key,len,locale,localizedDict,localizedText,methods,options,restore,self_translate,set_locale,translate,ver;ver='1.3.3';localizedDict=sup.localizedDict||{};localizedText={};locale=null;options={case_sensitive:false};if(typeof opts==='object'&&opts!==null){for(i=0,len=optinos.length;i<len;i++){key=optinos[i];if(key in opts){options[key]=opts[key];}}}
...
return loc_text_dict;};insert=function(loc,loc_texts){if(loc&&Array.isArray(loc_texts)){return localizedDict[loc.toString()]=loc_texts;}};append=function(new_text_list){var j,len1,text;if(!Array.isArray(new_text_list)){return;}
...
locale=null;loc=loc.replace('-','_');for(k in localizedDict){v=localizedDict[k];if(k.toLowerCase()===loc.toLowerCase()){locale=k;break;}}
if(locale&&localizedDict[locale]){locale_dict=localizedDict[locale];}else{locale_dict=null;locale=loc;}
...
params[k].push(v);}}}
return params;};angular.dataURLToBlob=function(dataURL){var BASE64_MARKER,contentType,i,parts,raw,rawLength,uInt8Array;BASE64_MARKER=';base64,';if(dataURL.indexOf(BASE64_MARKER)===-1){parts=dataURL.split(',');contentType=parts[0].split(':')[1];raw=parts[1];return new Blob([raw],{type:contentType});}
...
return new Blob([uInt8Array],{type:contentType});};angular.esc_regexp=function(str){return str.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');};if(typeof angular.translate!=='function'){angular.translate=function(str){var arg,args,l,len,len1,m,ref;if(typeof str!=='string'){return str;}
