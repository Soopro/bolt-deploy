...
return str.replace(/<[^>]+>/gm,'');};angular.clean_space=function(content,force){if(!force){return content.replace(/\s{2,}/g," ").trim();}else{return content.replace(" ","").trim();}};angular.title=function(text){return text.charAt(0).toUpperCase()+text.slice(1).toLowerCase();};angular.lower=function(text){return text.toLowerCase();};angular.upper=function(text){return text.toUpperCase();};angular.clean_ng_attr=function(content){return content.replace(/ng-[a-zA-Z0-9_\-]+=["\'].*?["\']/ig,'');};angular.isDict=function(obj){return Boolean(angular.isObject(obj)&&!angular.isArray(obj));};angular.isUrl=function(url,regex){if(typeof url!=='string'){return false;}
if(regex==='*'){regex=/^[\w]+:[a-zA-Z0-9\/]/i;}else if(!(regex instanceof RegExp)){regex=/^([\w]+:)?\/\/[a-zA-Z0-9]/i;}
return url.match(regex);};angular.addURLParams=function(url,params){var _add,item,k,l,len,v;if(!angular.isObject(params)){return url;}
