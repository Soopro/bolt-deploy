...
regex=/^[\w]+:/i;return Boolean(str.match(regex));};angular.addURLParams=function(url,params){var _add,item,k,l,len1,v;if(!angular.isObject(params)){return url;}
_add=function(url,key,value){var joint;joint=url.indexOf('?')>-1?'&':'?';key=encodeURIComponent(key);value=encodeURIComponent(value);url=url+joint+key+'='+value;return url;};for(k in params){v=params[k];if(angular.isArray(v)){for(l=0,len1=v.length;l<len1;l++){item=v[l];url=_add(url,k,item);}}else{url=_add(url,k,v);}}
return url;};angular.getURLParams=function(url,unique){var hash_idx,k,l,len1,pair,params,query_idx,query_vars,v;if(typeof url!=='string'){return{};}
if(unique===void 0){unique=true;}
hash_idx=url.indexOf('#');if(hash_idx>-1){url=url.substring(0,hash_idx);}
params={};query_idx=url.indexOf('?');if(query_idx>-1){query_vars=url.split('?')[1].split('&');for(l=0,len1=query_vars.length;l<len1;l++){v=query_vars[l];pair=v.split('=');k=pair[0];v=pair[1];if(!k){continue;}else if(!params[k]){params[k]=v;}else if(!unique){if(!angular.isArray(params[k])){params[k]=[params[k]];}
params[k].push(v);}}}
