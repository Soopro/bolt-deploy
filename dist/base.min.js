...
return historyManager.record();}};insertOrderedList=function(e){var content,node,ol;if(!inEditor(selectedNode,true)){return;}
...
return Boolean(find);};watchSelected=function(e){var tag_name,target;switchIndentOutdentBtns();target=findStylesParent(e.target||selectedNode);tag_name=target.tagName.toLowerCase();return targetText(target);};watchDoubleClick=function(e){var child_img,tag_name,target;if(selectedNode&&selectedNode.tagName==='A'){target=selectedNode;}else{target=findStylesParent(e.target||selectedNode);}
tag_name=target.tagName.toLowerCase();switch(tag_name){case'img':return targetImg(target);case'video':return targetVideo(target);case'audio':return targetAudio(target);case'a':child_img=target.querySelector('img');if(child_img){return targetImg(child_img);}else{return targetA(target);}}};switchIndentOutdentBtns=function(){getCurrentSelection();if(inElement(selectedNode,'LI')||inElement(selectedNode,'OL')){if($btn_indent){$btn_indent.removeAttribute('disabled');}
...
is_obj=o&&typeof o==='object'&&o!==null;is_obj_type=o.nodeType===1&&typeof o.nodeName==='string';result=is_obj&&is_obj_type;return result;};inElement=function(element,tag){if(!element||typeof tag!=='string'){return false;}
tag=tag.toUpperCase();if(element.tagName&&element.tagName===tag){return element;}else{if(element.parentElement||element===$editor){return inElement(element.parentElement,tag);}else{return false;}}};isFamily=function(elm1,elm2){var parent1,parent2;parent1=elm1.parentElement;parent2=elm2.parentElement;return elm1===elm2||parent1===elm2||parent2===elm1;};findStylesParent=function(element){var elementTagName,ref;elementTagName=element.tagName.toLowerCase();if(element.nodeType===1&&indexOf.call(styleTags,elementTagName)<0){return element;}
...
return element;};findTopElement=function(element){if(element.parentElement===$editor&&element.nodeType===1){return element;}else if(inEditor(element.parentElement)){return findTopElement(element.parentElement);}
...
results=[];for(l=0,len1=tags.length;l<len1;l++){tag=tags[l];empty_els=element.querySelectorAll(tag);results.push((function(){var len2,m,results1;results1=[];for(m=0,len2=empty_els.length;m<len2;m++){el=empty_els[m];if(!el.innerHTML){results1.push(element.removeChild(el));}else{results1.push(void 0);}}
return results1;})());}
return results;};toggleTextWrapper=function(target){var content,elm,regex;target=target.toLowerCase();regex=new RegExp('<[\/]?'+target+'.*?>','ig');if(selectedNode.tagName===target.toUpperCase()){elm=document.createElement('span');elm.appendChild(selRange.cloneContents());content=elm.innerHTML.replace(regex,'');selRange.deleteContents();pasteHtmlAtCaret(content,true);return true;}else{if(!isFamily(startRange.parentElement,endRange.parentElement)){console.info("Can not Wrapper text between different blocks.");return false;}
elm=document.createElement(target);elm.appendChild(selRange.cloneContents());elm.innerHTML=elm.innerHTML.replace(regex,'');if(!elm.innerHTML){return false;}
selRange.deleteContents();selRange.insertNode(elm);setRange(elm);return true;}
return false;};getDisplayType=function(element){var style;style=element.currentStyle||window.getComputedStyle(element,"");return style.display;};pasteHtmlAtCaret=function(html,selectPastedContent){var el,firstNode,frag,lastNode,node,originalRange,range,sel;if(window.getSelection){sel=window.getSelection();if(sel.getRangeAt&&sel.rangeCount>0){range=sel.getRangeAt(0);range.deleteContents();el=document.createElement('div');el.innerHTML=html;frag=document.createDocumentFragment();node=void 0;lastNode=void 0;while(node=el.firstChild){lastNode=frag.appendChild(node);}
...
range=document.createRange();range.setStartAfter(node);range.collapse(true);selection=window.getSelection();selection.removeAllRanges();return selection.addRange(range);};wildClear=function(){var current,end,i,resultObj,room,start,tmp_trigger,trigger;if(inEditor(selectedNode)){start=startRange.parentElement;end=endRange.parentElement;if(start!==end){resultObj=getSameParent(start,end);room=resultObj.point.childNodes;i=0;while(i<room.length){if(room[i].nodeType===1){tmp_trigger=false;if(room[i]===resultObj.start||room[i]===resultObj.end){if(trigger){tmp_trigger=false;}else{trigger=true;tmp_trigger=trigger;}}
...
if(inEditor(selectedNode)||force){tag=tag.toLowerCase();if(startRange.parentElement===endRange.parentElement){document.execCommand('formatBlock',false,tag);return historyManager.record();}}};execCmd=function(cmd,show_ui,str){getCurrentSelection();if(!inEditor(selectedNode,true)){return;}
...
return console.info('---------------------------');};assertParam=function(param,failback,type){var result;if(!failback){failback='';}
if(!type){type='string';}
result=typeof param===type?param:failback;return result;};isJsonString=function(str){var e;try{JSON.parse(str);}catch(error){e=error;return false;}
...
if(typeof target==='object'){if(typeof handler==='function'){target.removeEventListener(event,handler);target.removeEventListener(event,baseEventHanlder);}}}};resetCurrentMedia=function(){$currentImage={link:void 0,element:void 0};$currentVideo={element:void 0};return $currentAudio={element:void 0};};resetCurrentLink=function(){return $currentLink={element:void 0};};doInsertImg=function(imgObj){var alt,class_,class_str,height,href,html,size_str,src,target,target_str,width;src=assertParam(imgObj.src);alt=assertParam(imgObj.title);href=assertParam(imgObj.href);target=assertParam(imgObj.target);width=assertParam(imgObj.width);height=assertParam(imgObj.height);class_=assertParam(imgObj.class);if(class_){class_str='class="'+class_+'"';}else{class_str='';}
...
html='<a '+class_str+' '+target_str+' href="'+href+'">'+'<img src="'+src+'" '+size_str+' alt="'+alt+'"></a>';}else{html='<img '+class_str+' '+size_str+' src="'+src+'" alt="'+alt+'" />';}
return html;};doReplaceImg=function(imgObj){var alt,class_,class_str,height,href,img,src,target,target_str,width;img=$currentImage.element;if(!img||!imgObj){return;}
src=assertParam(imgObj.src);alt=assertParam(imgObj.title);href=assertParam(imgObj.href);target=assertParam(imgObj.target);width=assertParam(imgObj.width);height=assertParam(imgObj.height);class_=assertParam(imgObj.class);if(src){img.setAttribute('src',src);img.removeAttribute('ng-src');}
...
return img;}; doInsertVideo=function(videoObj){var autoplay,class_,class_str,controls,html,looping,poster,src;src=assertParam(videoObj.src);poster=assertParam(videoObj.poster);controls=videoObj.controls?'controls':'';looping=videoObj.loop?'loop':'';autoplay=videoObj.autoplay?'autoplay':'';class_=assertParam(videoObj.class);if(class_){class_str='class="'+class_+'"';}else{class_str='';}
html='<video '+looping+' '+autoplay+' '+controls+' '+class_str+' '+'src="'+src+'" '+' poster="'+poster+'" preload="none"></video>';return html;};doReplaceVideo=function(videoObj){var autoplay,class_,controls,looping,poster,src,video;video=$currentVideo.element;if(!video||!videoObj){return;}
src=assertParam(videoObj.src);poster=assertParam(videoObj.poster);controls=videoObj.controls?'controls':'';looping=videoObj.loop?'loop':'';autoplay=videoObj.autoplay?'autoplay':'';class_=assertParam(videoObj.class);if(src){video.setAttribute('src',src);video.removeAttribute('ng-src');}
...
return video;};doInsertAudio=function(audioObj){var autoplay,class_,class_str,controls,html,looping,src;src=assertParam(audioObj.src);controls='controls';looping=audioObj.loop?'loop':'';autoplay=audioObj.autoplay?'autoplay':'';class_=assertParam(audioObj.class);if(class_){class_str='class="'+class_+'"';}else{class_str='';}
html='<audio '+looping+' '+autoplay+' '+controls+' '+class_str+' '+'src="'+src+'"></audio>';return html;};doReplaceAudio=function(audioObj){var audio,autoplay,class_,controls,looping,src;audio=$currentAudio.element;if(!audio||!audioObj){return;}
src=assertParam(audioObj.src);controls=audioObj.controls?'controls':'';looping=audioObj.loop?'loop':'';autoplay=audioObj.autoplay?'autoplay':'';class_=assertParam(audioObj.class);if(src){audio.setAttribute('src',src);audio.removeAttribute('ng-src');}
...
href=assertParam(linkObj.href);target=assertParam(linkObj.target);class_=assertParam(linkObj.class);restoreSelection(savedRange);if(savedRange.endOffset>0){execCmd('createLink',false,href);getCurrentSelection();a=selectedNode;if(a.tagName==='A'){if(target){a.setAttribute('target',target);}
if(class_){a.setAttribute('class',class_);}}}else{if(target){target_str='target="'+target+'"';}else{target_str='';}
if(class_){class_str='class="'+class_+'"';}else{class_str='';}
name=assertParam(linkObj.name,href);link='<a '+class_str+' '+target_str+' href="'+href+'">'+name+'</a>';pasteHtmlAtCaret(link);}
historyManager.record();return dispatchSAWEvent();};execReplaceLink=function(linkObj){var a,class_,href,target;a=$currentLink.element;if(!a||!linkObj){return;}
href=assertParam(linkObj.href);target=assertParam(linkObj.target);class_=assertParam(linkObj.class);restoreSelection(savedRange);a.removeAttribute('ng-href');if(href){a.setAttribute('href',href);}
...
resetCurrentLink();historyManager.record();return dispatchSAWEvent();};execRemoveLink=function(){var a;a=$currentLink.element;if(!a){return;}
...
if(typeof preset.code!=='string'){preset.code='';}
restoreSelection(savedRange);preset_element=document.createElement('div');preset_element.setAttribute('preset-id',Date.now());preset_element.innerHTML=preset.code;element=findTopElement(selectedNode);if(element&&element.nextSibling){$editor.insertBefore(preset_element,element.nextSibling);}else{$editor.appendChild(preset_element);}
historyManager.record();return dispatchSAWEvent();};execInsertScript=function(script){if(!savedRange||!script){return;}
if(typeof script!=='string'){script='';}
restoreSelection(savedRange);pasteHtmlAtCaret(script);historyManager.record();return dispatchSAWEvent();};init_editor=function(editor,toolbar,opt){var _history,e;load_options(opt);$document=document;if(typeof editor==='string'&&editor!==''){$editor=$document.querySelector('[id='+editor+']');}else if(isHTMLElement(editor)){$editor=editor;}
...
$editor.setAttribute('contenteditable','true');if($toolbar){$toolbar=createToolbarBtns($toolbar);}
...
presets=[];if(angular.isArray(scope.presets)){ref=scope.presets;for(l=0,len1=ref.length;l<len1;l++){p=ref[l];presets.push({preview:p.preview,code:p.code});}}
...
return new Blob([uInt8Array],{type:contentType});};if(typeof angular.translate!=='function'){angular.translate=function(str){var arg,args,l,len,len1,m,ref;if(typeof str!=='string'){return str;}
