...
return output;};});}).call(this);(function(){var indexOf=[].indexOf;angular.module('boltApp').directive('editorCanvas',['$compile',function($compile){return{restrict:'AE',scope:{},controller:['$scope','$rootScope','$window','$element','$compile','$interval','$timeout','supLocales',function($scope,$rootScope,$window,$element,$compile,$interval,$timeout,supLocales){var check_keymap,clean_saw_complete,compile_tpl,content_type,curr_editor,editor_metas,keymap,last_tpl,render,restEditor,root,sawStack,station,sup_template_filter,vintage_mode,vintage_tmpl_pretreat;$scope._=angular.translate;if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){console.error("Canvas must running in an station.");return;}
curr_editor=station.editor_context;restEditor=station.restEditor;vintage_mode=station.vintage_mode;if(vintage_mode){console.info('Canvas running in Vintage mode.');}
$scope.g={};$scope.query={};editor_metas=curr_editor.metas;$scope.base_url=$rootScope.base_url=editor_metas.base_url;$scope.theme_url=$rootScope.theme_url=editor_metas.theme_url;$scope.res_url=$rootScope.res_url=editor_metas.res_url;$scope.locale=$rootScope.locale=editor_metas.locale;$scope.lang=$rootScope.lang=editor_metas.lang;$scope.site_meta=editor_metas.site_meta;$scope.theme_meta=editor_metas.theme_meta;$scope.menu=editor_metas.menu;$scope.languages=editor_metas.languages;$scope.socials=editor_metas.socials;$scope.slot=editor_metas.slot;content_type=editor_metas.content_type[station.curr_content_type];$scope.content_type=content_type;$scope.visit=curr_editor.visit;compile_tpl=function(app_slug,template){station.display_canvas(false);return restEditor.tpl.get({app_slug:app_slug,tmpl_slug:template},function(data){var compile,e,timer;try{supLocales.load(curr_editor.app.locale,data.localized_dict);}catch(error){e=error;console.error('Canvas localize error:',e);}
$element.html(data.tpl);sup_template_filter($element[0]);if(vintage_mode){vintage_tmpl_pretreat($element[0]);}
...
return el;};vintage_tmpl_pretreat=function(el){var i,len,saw,saw_editors;saw_editors=el.querySelectorAll('[sup-angular-wysiwyg]')||[];for(i=0,len=saw_editors.length;i<len;i++){saw=saw_editors[i];saw.setAttribute('readonly',true);}
return el;};last_tpl=null;$scope.$watch('file.template',function(new_tpl){if(last_tpl===new_tpl||!new_tpl){return;}
compile_tpl(curr_editor.app.slug,new_tpl);return last_tpl=new_tpl;});$scope.file=station.file;$scope.$watch('file',function(file){console.log(file);$scope.content=file.content;$scope.meta=file.meta||{};$scope.meta.id=file.id;$scope.meta.url=file.url;$scope.meta.path=file.path;$scope.meta.priority=file.priority;$scope.meta.parent=file.parent;$scope.meta.type=$scope.meta.content_type=file.content_type;$scope.meta.status=file.status;$scope.meta.updated=file.updated;$scope.meta.creation=file.creation;$scope.meta.date=file.date;$scope.meta.price=file.price;$scope.meta.template=file.template;$scope.meta.tags=file.tags;$scope.meta.terms=file.terms;return $scope.meta.shelf=file.shelf;},true);$scope.$watch('content',function(value){return $scope.file.content=value;});if(vintage_mode){$rootScope.escape=true;}else{keymap={};check_keymap=function(codes){var code,i,len;if(typeof codes==='number'){codes=[codes];}
...
if(check_keymap([17,27])){escape=!$rootScope.escape;station.switch_sidebar(!escape);$rootScope.escape=escape;e.preventDefault();return console.info('Editor: escape '+escape);}}else if(e.type==='keyup'){return keymap[e.keyCode]=false;}});}
$element.attr('origin-class',$element.attr('class'));sawStack=[];clean_saw_complete=$rootScope.$on('saw.loaded',function(e,saw){if(typeof saw.destroy==='function'){return sawStack.push(saw);}});$scope.$on('$destroy',function(){var i,len,saw;for(i=0,len=sawStack.length;i<len;i++){saw=sawStack[i];saw.destroy();}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
scope.options={allow_types:scope.allowTypes||attrs.supWidgetBg,allow_fields:angular.str2opts(scope.allowFields),presets:scope.presets};element.on("mousedown",function(e){if(station.vintage_mode||e.target!==element[0]){return;}
...
scope.options={structure:scope.structure};element.on("mousedown",function(e){if(station.vintage_mode){return;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
scope.options={link_to_type:scope.linkToType,allow_fields:angular.str2opts(scope.allowFields)};element.on("mousedown",function(e){if(station.vintage_mode){return;}
...
return element.on("mousedown",function(e){if(station.vintage_mode){return;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
default_data={};scope.options={};element.on("mousedown",function(e){if(station.vintage_mode){return;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
scope.options={limit:Math.min(angular.parse_int(scope.limit,12,true),24),opt_limit:Math.min(angular.parse_int(scope.optLimit,6,true),24)};element.on("mousedown",function(e){if(station.vintage_mode){return;}
...
if(!angular.isArray(default_data)){default_data=[];}
scope.limit=Math.min(angular.parse_int(scope.limit,12,true),24);scope.options={limit:scope.limit};element.on("mousedown",function(e){if(station.vintage_mode){return;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
scope.options={allow_types:scope.allowTypes||attrs.supWidgetMedia,allow_fields:angular.str2opts(scope.allowFields),resizable:angular.isDefined(attrs.resizable),lite:angular.isDefined(attrs.lite)};element.on("mousedown",function(e){if(station.vintage_mode){return;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
scope.vintage_mode=station.vintage_mode;app=station.editor_context.app;file=station.file;restEditor=station.restEditor;scope.limit=Math.min(angular.parse_int(scope.limit,24,true),24);scope.data=[];update_add_element=function(data){var el_add;if(!data||!data.length){return;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
scope.vintage_mode=station.vintage_mode;try{_default=scope.$eval(scope.default);default_data=[];for(i=0,len=_default.length;i<len;i++){item=_default[i];default_data.push({src:item.src||'',touch:item.touch||'',type:item.type||'image',title:item.title||'',subtitle:item.subtitle||'',caption:item.caption||'',link:'',target:'',class:'',poster:item.poster||'',extra:item.extra||'',tags:item.tags||[]});}}catch(error){e=error;default_data=[];}
if(!angular.isArray(default_data)){default_data=[];}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
dirty=false;if(!station.vintage_mode){element[0].setAttribute('contenteditable','true');}
if(scope.default){default_data=scope.default;}else{default_data=$interpolate(element.html().trim())($rootScope);}
element.on("click mousedown",function(e){if(station.vintage_mode){return;}
...
return dirty=true;};element.on("blur keyup change cut",function(e){if(station.vintage_mode){return;}
return readViewText();});clear_html=function(html){html=html.replace(/<div\s*.*?>/ig,'<br>');html=html.replace(/<(?!br\s*\/?)[^>]+>/ig,'');if(html==="<br>"){html="";}
return html;};clear_style=function(elm){var el,i,len,ref;elm.removeAttribute('style');ref=elm.children;for(i=0,len=ref.length;i<len;i++){el=ref[i];clear_style(el);}};element.on("paste",function(e){var text;if(station.vintage_mode){return;}
...
scope.options={allow_fields:angular.str2opts(scope.allowFields)};element.on("mousedown",function(e){if(station.vintage_mode){return;}
