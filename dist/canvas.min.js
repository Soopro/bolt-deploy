...
$scope.g={};$scope.query={};return timer=$timeout(function(){try{return station.display_canvas(true);}catch(error){e=error;return console.error(e);}},600);});};sup_template_filter=function(el){var i,len,templates,tmpl,tmpl_id,tmpl_script;templates=el.querySelectorAll('sup-template')||[];for(i=0,len=templates.length;i<len;i++){tmpl=templates[i];tmpl_id=tmpl.getAttribute('id');tmpl_script=document.createElement('script');tmpl_script.setAttribute('id',tmpl_id);tmpl_script.setAttribute('type','text/ng-template');tmpl_script.innerHTML=tmpl.innerHTML;tmpl.parentNode.replaceChild(tmpl_script,tmpl);}
