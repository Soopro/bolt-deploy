...
scope._=angular.translate;return $compile(element.html(tmpl_content).contents())(scope);}};}]).directive('supBody',[function(){return{restrict:'AE',compile:function(){return function(scope,element,attrs){var doc_body_cls,doc_body_style;doc_body_cls=document.body.getAttribute('class');doc_body_style=document.body.getAttribute('style');document.body.setAttribute('style',attrs.style);document.body.setAttribute('class',doc_body_cls+' '+attrs.class);element.addClass('ng-hide');return element.on('$destroy',function(){if(doc_body_cls){document.body.setAttribute('class',doc_body_cls);}else{document.body.removeAttribute('class');}
if(doc_body_style){return document.body.setAttribute('style',doc_body_style);}else{return document.body.removeAttribute('style');}});};}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetBg',['$rootScope','$window',function($rootScope,$window){return{scope:{allowTypes:'@',allowFields:'@',presets:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var loaded,root,station;if(!ngModel){return;}
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
