...
]).directive('supBindHtml',function(){return{restrict:'A',compile:function(){return function(scope,element,attrs){return scope.$watch(attrs.supBindHtml,function(val){return element.html(val||'');});};}};}).directive('supTempalte',[function(){return{scope:{id:'@'},restrict:'E',transclude:true,replace:true,template:['<script type="text/ng-template" id="{{id}}">','<ng-transclude></ng-transclude>','</script>'].join('')};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetBg',['$rootScope',function($rootScope){return{scope:{allowTypes:'@',allowFields:'@',presets:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var loaded,root,station;if(!ngModel){return;}
if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
