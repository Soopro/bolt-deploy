(function(){angular.module('boltApp',['ngSanitize','supLocales','supAngularWysiwyg']);}).call(this);(function(){angular.module('boltApp').constant('config',{baseURL:{'api':sup.conf.api,'apps':sup.conf.apps,'assistant':sup.conf.assistant},preview_cookie_domain:sup.conf.preview_cookie_domain,debug:sup.debug,browser:angular.browser(),global_media_repos:{g:'g',crm:'g.crm',erp:'g.erp'},cdn:{engine:'?imageMogr2',upload:window.location.protocol+'//upload.qbox.me'},locales:[{code:'en_US',name:'English'},{code:'zh_CN',name:'简体中文'}],default_locale:'en_US',social_medias:[{code:'weibo',name:'Weibo'},{code:'wechat',name:'WeChat'},{code:'douban',name:'Douban'},{code:'github',name:'Github'},{code:'behance',name:'Behance'},{code:'twitter',name:'Twitter'},{code:'facebook',name:'Facebook'},{code:'linkedin',name:'Linkedin'},{code:'vimeo',name:'Vimeo'},{code:'youtube',name:'YouTube'},{code:'google',name:'Google+'},{code:'other',name:'Other'}],media_icons:{'image':'image:ic_photo_24px','video':'av:ic_videocam_24px','audio':'image:ic_audiotrack_24px','application':'file:ic_attachment_24px'},max_image_size:{width:1650,height:1650},max_file_size:{image:1*1024*1024,video:24*1024*1024,audio:6*1024*1024,application:6*1024*1024},media_encoder_options:0.80,media_mimetypes:{image:['image/png','image/jpeg','image/svg+xml','image/gif'],video:['video/mp4','video/quicktime'],audio:['audio/mpeg','audio/mp3'],application:['application/zip','application/pdf']},expert_capability:6,capabilities:[{key:1,label:'Noob'},{key:2,label:'Junior'},{key:3,label:'Senior'},{key:6,label:'Expert',expert:true}],route:{index:'/app',info:'/info',profile:'/user/profile',oauth:'/user/oauth',auth:'/gate/login',exit:'/gate/exit',station:'/station',error:'/404'},paths:{upload:['/app/wa','/app/ws','/app/wx','/app/wxa','/crm','/erp/commodity'],gate:['/gate/login','/gate/register','/gate/recovery'],free:['/info','/404']},app_limit:60,app_types:[{name:'Brand Website',key:'ws',ico:'av:ic_web_24px',capability:0,station:'modern'},{name:'WeChat Buzz',key:'wx',ico:'action:ic_speaker_notes_24px',capability:0,station:'modern'},{name:'WeChat Mini Store',key:'wxa',ico:'action:ic_store_24px',capability:3,station:'strict'},{name:'Mobile Showreel',key:'wr',ico:'hardware:ic_phone_android_24px',capability:0,station:'modern'}],default_app_type:'ws',contents_perpage:12,category_terms_limit:120,email:{support:'support@soopro.com'},copyright:"Humbly powered by soopro."});}).call(this);(function(){angular.module('boltApp').filter("thumbnail",['config',function(config){return function(pic_url,preset_name){var engine_str,preset_map,suffix;if(!angular.isUrl(pic_url)){return pic_url;}
...
]).directive('supQuery',['$rootScope',function($rootScope){return{scope:{attrs:'=',contentType:'=',sortby:'=',perpage:'=',paged:'=',term:'=',withContent:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var app,data_query,queries,query_limit,restEditor,root,station;if(!ngModel){return;}
...
app=station.editor_context.app;restEditor=station.restEditor;data_query={attrs:angular.list(scope.attrs),content_type:scope.contentType||attrs.supQuery,sortby:angular.list(scope.sortby),perpage:angular.parse_int(scope.perpage),paged:angular.parse_int(scope.paged),term:scope.term,with_content:angular.bool(scope.withContent)};return restEditor.doEditorQuery(app.id,data_query).then(function(data){return ngModel.$setViewValue(data);});}};}
]).directive('supCategorize',['$rootScope',function($rootScope){return{scope:{termKeys:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var app,get_category,queries,query_limit,restEditor,root,station,submitted;if(!ngModel){return;}
query_limit=1;queries=document.querySelectorAll('[sup-categorize], sup-categorize');if(queries.length>query_limit){console.error('Query overrun.',element[0]);return;}
if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
...
return scope.data=ngModel.$viewValue;};loaded={label:attrs.label||attrs.name,key:attrs.ngModel};return station.$emit('widget.attrs.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetLink',['$rootScope',function($rootScope){return{scope:{linkToType:'@',allowFields:'@'},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var loaded,root,station;if(!ngModel){return;}
if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
...
station.$emit('widget.link.update',scope);return e.stopPropagation();});scope.update=function(data){scope.data=data;ngModel.$setViewValue(scope.data);return ngModel.$render();};ngModel.$render=function(){return scope.data=ngModel.$viewValue;};loaded={label:attrs.label||attrs.name,key:attrs.ngModel};return station.$emit('widget.link.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetCarousel',['$rootScope',function($rootScope){return{scope:{default:'@',allowTypes:'@',allowFields:'@',limit:'@'},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var _default,clip,default_data,e,i,len,loaded,root,station;if(!ngModel){return;}
if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
...
return scope.data=ngModel.$viewValue;};loaded={label:attrs.label||attrs.name,key:attrs.ngModel};return station.$emit('widget.carousel.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetCatalog',['$rootScope',function($rootScope){return{scope:{contentType:'@',itemsLimit:'@',limit:'@'},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var default_data,loaded,root,station;if(!ngModel){return;}
if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
...
scope.data=data||default_data;ngModel.$setViewValue(scope.data);return ngModel.$render();};element.on('$destroy',function(){ngModel.$render=angular.noop;if(angular.equals(ngModel.$viewValue,default_data)){return ngModel.$setViewValue(null);}});ngModel.$render=function(){if(!angular.isArray(ngModel.$viewValue)){ngModel.$setViewValue(default_data);}
return scope.data=ngModel.$viewValue;};loaded={label:attrs.label||attrs.name,key:attrs.ngModel};return station.$emit('widget.catalog.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetCreate',['$rootScope',function($rootScope){return{scope:{type:'@',parent:'@',template:'@'},restrict:'AE',link:function(scope,element,attrs){var root,station;if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
...
return scope.data=ngModel.$viewValue;};loaded={label:attrs.label||attrs.name,key:attrs.ngModel};return station.$emit('widget.bulletin.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetForm',['$rootScope',function($rootScope){return{scope:{limit:'@',optLimit:'@'},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var loaded,root,station;if(!ngModel){return;}
if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
...
return scope.data=ngModel.$viewValue;};loaded={label:attrs.label||attrs.name,key:attrs.ngModel};return station.$emit('widget.lines.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetMedia',['$rootScope',function($rootScope){return{scope:{allowTypes:'@',allowFields:'@'},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var loaded,root,station;if(!ngModel){return;}
if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
...
return element.on("mousedown",function(e){segmentsCtrl.$createSegment(scope.template);return e.stopPropagation();});}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetSeries',['$rootScope',function($rootScope){return{scope:{default:'@',allowTypes:'@',allowFields:'@',limit:'@'},restrict:'AE',require:'ngModel',controller:['$scope',function($scope){this.$editSeriesItem=function(index){if($scope.strict_mode){return;}
...
loaded={label:attrs.label||attrs.name,key:'shelf'};return station.$emit('widget.shelf.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetSlot',['$rootScope',function($rootScope){return{scope:{key:"@",allowFields:'@'},restrict:'AE',link:function(scope,element,attrs){var loaded,root,station;if(window.parent){root=parent.angular.element(parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station||!station.edit_slot){return;}
scope.options={allow_fields:angular.str2opts(scope.allowFields)};element.on("mousedown",function(e){return station.strict_mode;station.edit_slot(scope.key,scope.options);return e.stopPropagation();});loaded={label:attrs.label||attrs.name,key:scope.key};return station.$emit('widget.slot.loaded',loaded,scope);}};}]);}).call(this);