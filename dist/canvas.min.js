...
]).directive('supQuery',['$rootScope','$window',function($rootScope,$window){return{scope:{attrs:'=',contentType:'=',sortby:'=',perpage:'=',paged:'=',term:'=',withContent:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var app,data_query,queries,query_limit,restEditor,root,station;if(!ngModel){return;}
...
]).directive('supScriptLoader',['$window',function($window){return{restrict:'AE',replace:true,compile:function(element,attrs){var el_s;if(!attrs.src||typeof attrs.src!=='string'){return;}
el_s=$window.document.createElement('script');el_s.setAttribute('ng-src',"{{'"+attrs.src+"'}}");return element.append(el_s);}};}
...
]).directive('supBindHtml',function(){return{restrict:'A',compile:function(){return function(scope,element,attrs){return scope.$watch(attrs.supBindHtml,function(val){return element.html(val||'');});};}};}).directive('supTempalte',[function(){return{scope:{id:'@'},restrict:'E',transclude:true,replace:true,template:['<script type="text/ng-template" id="{{id}}">','<ng-transclude></ng-transclude>','</script>'].join('')};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetBg',['$rootScope','$window',function($rootScope,$window){return{scope:{allowTypes:'@',allowFields:'@',presets:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var loaded,root,station;if(!ngModel){return;}
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
...
return element.on("mousedown",function(e){seriesCtrl.$addSeriesItem();return e.stopPropagation();});}};}]);}).call(this);(function(){angular.module('boltApp').directive("supWidgetText",['$rootScope','$timeout','$interpolate','$window',function($rootScope,$timeout,$interpolate,$window){return{scope:{default:"@"},restrict:"AE",require:"?ngModel",link:function(scope,element,attrs,ngModel){var DOC_EL,clear_html,clear_style,default_data,dirty,insertTextAtCursor,loaded,readViewText,root,station;if(!ngModel){return;}
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
DOC_EL=$window.document;station=root.station;if(!station){return;}
...
return html;};clear_style=function(elm){var el,i,len,ref;elm.removeAttribute('style');ref=elm.children;for(i=0,len=ref.length;i<len;i++){el=ref[i];clear_style(el);}};insertTextAtCursor=function(text){var html,range,sel;sel=void 0;range=void 0;html=void 0;if($window.getSelection){sel=$window.getSelection();if(sel.getRangeAt&&sel.rangeCount){range=sel.getRangeAt(0);range.deleteContents();range.insertNode(DOC_EL.createTextNode(text));}}else if(DOC_EL.selection&&DOC_EL.selection.createRange){DOC_EL.selection.createRange().text=text;}};element.on("paste",function(e){var text;if(station.strict_mode){return;}
if(e.clipboardData&&e.clipboardData.getData){e.preventDefault();text=e.clipboardData.getData("text/plain");return DOC_EL.execCommand("insertHTML",false,text);}else if($window.clipboardData&&$window.clipboardData.getData){text=$window.clipboardData.getData("Text");return insertTextAtCursor(text);}});readViewText=function(){var html;html=clear_html(element.html());if(html==="<br>"){html="";}
