...
return is_empty;};}).filter("price",function(){return function(val,use_currency,fractionSize){return angular.price(val,use_currency,fractionSize);};});}).call(this);(function(){angular.module('boltApp').config(['$sceDelegateProvider',function($sceDelegateProvider){return $sceDelegateProvider.resourceUrlWhitelist(['self','http://**.soopro.io/**','https://**.soopro.io/**','http://**.soopro.com/**','https://**.soopro.com/**']);}]);}).call(this);(function(){var indexOf=[].indexOf;angular.module('boltApp').filter("trust",['$sce',function($sce){return function(str){return $sce.trustAsHtml(str);};}]).filter("capitalize",function(){return function(str){return angular.capitalize(str);};}).filter("url",['$rootScope',function($rootScope){return function(url,remove_args,remove_hash){var index;if(typeof url!=='string'){return url;}
...
_check_active=function(item){var path_scope;if(path===menu_item.path){return true;}else if(menu_item.path_scope){path_scope=angular.lstrip(menu_item.path_scope,'/');if(angular.startswith(path,'/'+path_scope)){return true;}}
...
return output;};});}).call(this);(function(){var indexOf=[].indexOf;angular.module('boltApp').directive('editorCanvas',['$compile',function($compile){return{restrict:'AE',scope:{},controller:['$scope','$rootScope','$window','$element','$compile','$interval','$timeout','supLocales',function($scope,$rootScope,$window,$element,$compile,$interval,$timeout,supLocales){var check_keymap,clean_saw_complete,compile_tpl,content_type,curr_editor,editor_metas,keymap,last_tpl,render,restEditor,root,sawStack,station,sup_template_filter;$scope._=angular.translate;if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){console.error("Canvas must running in an station.");return;}
curr_editor=station.editor_context;restEditor=station.restEditor;$scope.g={};$scope.query={};editor_metas=curr_editor.metas;$scope.base_url=$rootScope.base_url=editor_metas.base_url;$scope.theme_url=$rootScope.theme_url=editor_metas.theme_url;$scope.res_url=$rootScope.res_url=editor_metas.res_url;$scope.locale=$rootScope.locale=editor_metas.locale;$scope.lang=$rootScope.lang=editor_metas.lang;$scope.site_meta=editor_metas.site_meta;$scope.theme_meta=editor_metas.theme_meta;$scope.menu=editor_metas.menu;$scope.languages=editor_metas.languages;$scope.socials=editor_metas.socials;$scope.slot=editor_metas.slot;content_type=editor_metas.content_type[station.curr_content_type];$scope.content_type=content_type;$scope.visit=curr_editor.visit;compile_tpl=function(app_slug,template){station.display_canvas(false);return restEditor.tpl.get({app_slug:app_slug,tmpl_slug:template},function(data){var compile,e,timer;try{supLocales.load(curr_editor.app.locale,data.localized_dict);}catch(error){e=error;console.error('Canvas localize error:',e);}
$element.html(data.tpl);sup_template_filter($element[0]);try{compile=$compile($element.contents());compile($scope);}catch(error){e=error;console.error('Canvas rendering error:',e);}
...
compile_tpl(curr_editor.app.slug,new_tpl);return last_tpl=new_tpl;});$scope.file=station.file;$scope.$watch('file',function(file){$scope.content=file.content;$scope.meta=file.meta||{};$scope.meta.id=file.id;$scope.meta.url=file.url;$scope.meta.path=file.path;$scope.meta.priority=file.priority;$scope.meta.parent=file.parent;$scope.meta.type=$scope.meta.content_type=file.content_type;$scope.meta.status=file.status;$scope.meta.updated=file.updated;$scope.meta.creation=file.creation;$scope.meta.date=file.date;$scope.meta.template=file.template;$scope.meta.tags=file.tags;return $scope.meta.terms=file.terms;},true);$scope.$watch('content',function(value){return $scope.file.content=value;});keymap={};check_keymap=function(codes){var code,i,len;if(typeof codes==='number'){codes=[codes];}
for(i=0,len=codes.length;i<len;i++){code=codes[i];if(!keymap[code]){return false;}}
return true;};$element.on('click blur focus',function(e){return keymap={};});$element.on('keydown keyup',function(e){var escape,key_codes,ref;key_codes=[17,27,91,83];if(ref=e.keyCode,indexOf.call(key_codes,ref)<0){return;}
if(e.type==='keydown'){if(keymap[e.keyCode]){return;}
keymap[e.keyCode]=e.type==='keydown';if(check_keymap([91,83])){station.save();keymap[91]=false;keymap[83]=false;e.preventDefault();e.stopPropagation();console.info('Editor: Try to Save');}
if(check_keymap([17,27])){escape=!$rootScope.escape;station.switch_sidebar(!escape);$rootScope.escape=escape;e.preventDefault();return console.info('Editor: escape '+escape);}}else if(e.type==='keyup'){return keymap[e.keyCode]=false;}});$element.attr('origin-class',$element.attr('class'));sawStack=[];clean_saw_complete=$rootScope.$on('saw.loaded',function(e,saw){if(typeof saw.destroy==='function'){return sawStack.push(saw);}});$scope.$on('$destroy',function(){var i,len,saw;for(i=0,len=sawStack.length;i<len;i++){saw=sawStack[i];saw.destroy();}
...
]).directive('supBindHtml',[function(){return{restrict:'A',compile:function(){return function(scope,element,attrs){return scope.$watch(attrs.supBindHtml,function(val){return element.html(val||'');});};}};}
]).directive('supLoadTemplate',['$compile','$templateCache',function($compile,$templateCache){return{scope:{supLoadTemplate:'=',templateId:'=',importWithContext:'='},restrict:'AE',link:function(scope,element,attrs){var k,ref,tmpl_content,tmpl_id,v;tmpl_id=scope.supLoadTemplate||scope.templateId;tmpl_content=$templateCache.get(tmpl_id)||'';if(angular.isObject(scope.importWithContext)){ref=scope.importWithContext;for(k in ref){v=ref[k];scope[k]=v;}}
scope._=angular.translate;return $compile(element.html(tmpl_content).contents())(scope);}};}]).directive('supBody',[function(){return{scope:{class:'@',style:'@'},restrict:'AE',link:function(scope,element){var origin_cls;origin_cls=document.body.getAttribute('origin-class');if(!element.hasClass('ng-hide')){element.addClass('ng-hide');}
scope.$watchGroup(['class','style'],function(){document.body.setAttribute('style',scope.style);return document.body.setAttribute('class',origin_cls+' '+scope.class);});return element.on('$destroy',function(){document.body.setAttribute('class',origin_cls);return document.body.removeAttribute('style');});}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetBg',['$rootScope','$window',function($rootScope,$window){return{scope:{allowTypes:'@',allowFields:'@',presets:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var loaded,root,station;if(!ngModel){return;}
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
scope.options={allow_types:scope.allowTypes||attrs.supWidgetBg,allow_fields:angular.str2opts(scope.allowFields),presets:scope.presets};element.on("mousedown",function(e){if(e.target!==element[0]){return;}
...
scope.limit=Math.min(angular.parse_int(scope.limit,12,true),24);scope.options={allow_types:scope.allowTypes||attrs.supWidgetCarousel,allow_fields:angular.str2opts(scope.allowFields),link_to_type:attrs.linkToType,limit:scope.limit,lite:angular.isDefined(attrs.lite)};element.on("mousedown",function(e){station.$emit('widget.carousel.update',scope);return e.stopPropagation();});scope.update=function(data){if(!data||data.length===0){data=null;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
default_data=[];scope.limit=Math.min(angular.parse_int(scope.limit,6,true),12);scope.items_limit=angular.parse_int(scope.itemsLimit,6,true);scope.items_limit=Math.min(scope.items_limit,12);scope.options={content_type:scope.contentType,items_limit:scope.items_limit,limit:scope.limit};element.on("mousedown",function(e){station.$emit('widget.catalog.update',scope);return e.stopPropagation();});scope.update=function(data){if(!data||data.length===0){data=null;}
scope.data=data||default_data;ngModel.$setViewValue(scope.data);return ngModel.$render();};element.on('$destroy',function(){ngModel.$render=angular.noop;if(angular.equals(ngModel.$viewValue,default_data)){return ngModel.$setViewValue(null);}});ngModel.$render=function(){if(!angular.isArray(ngModel.$viewValue)){ngModel.$setViewValue(default_data);}
...
if(!angular.isArray(default_data)){default_data=[];}
scope.limit=Math.min(angular.parse_int(scope.limit,12,true),24);scope.options={limit:scope.limit};element.on("mousedown",function(e){station.$emit('widget.lines.update',scope);return e.stopPropagation();});scope.update=function(data){scope.data=data||default_data;ngModel.$setViewValue(scope.data);return ngModel.$render();};element.on('$destroy',function(){ngModel.$render=angular.noop;if(angular.equals(ngModel.$viewValue,default_data)){return ngModel.$setViewValue(null);}});ngModel.$render=function(){if(!angular.isArray(ngModel.$viewValue)){ngModel.$setViewValue(default_data);}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
app=station.editor_context.app;file=station.file;restEditor=station.restEditor;scope.limit=Math.min(angular.parse_int(scope.limit,24,true),24);scope.data=[];update_add_element=function(data){var el_add;if(!data||!data.length){return;}
...
if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station){return;}
dirty=false;element[0].setAttribute('contenteditable','true');if(scope.default){default_data=scope.default;}else{default_data=$interpolate(element.html().trim())($rootScope);}
element.on("click mousedown",function(e){station.$emit('widget.text.touch',scope);return e.stopPropagation();});ngModel.$render=function(){var timer;if(!ngModel.$viewValue&&!dirty){element.html(default_data);timer=$timeout(function(){readViewText();return $timeout.cancel(timer);},0);}else{element.html(ngModel.$viewValue||"");}
return dirty=true;};element.on("blur keyup change cut",function(e){return readViewText();});clear_html=function(html){html=html.replace(/<div\s*.*?>/ig,'<br>');html=html.replace(/<(?!br\s*\/?)[^>]+>/ig,'');if(html==="<br>"){html="";}
return html;};clear_style=function(elm){var el,i,len,ref;elm.removeAttribute('style');ref=elm.children;for(i=0,len=ref.length;i<len;i++){el=ref[i];clear_style(el);}};element.on("paste",function(e){var text;if(e.clipboardData&&e.clipboardData.getData){e.preventDefault();text=e.clipboardData.getData("text/plain");return $window.document.execCommand("insertText",false,text);}else if($window.clipboardData&&$window.clipboardData.getData){e.preventDefault();text=$window.clipboardData.getData("Text");return $window.document.execCommand("insertText",false,text);}});readViewText=function(){var html;html=clear_html(element.html());if(html==="<br>"){html="";}
return ngModel.$setViewValue(html.trim());};loaded={label:attrs.label||attrs.name,key:attrs.ngModel};return station.$emit('widget.text.loaded',loaded,scope);}};}]);}).call(this);(function(){angular.module('boltApp').directive('supWidgetSlot',['$rootScope','$window',function($rootScope,$window){return{scope:{key:"@",allowFields:'@'},restrict:'AE',link:function(scope,element,attrs){var loaded,root,station;if($window.parent){root=$window.parent.angular.element($window.parent.document).scope();}else{root=$rootScope;}
station=root.station;if(!station||!station.edit_slot){return;}
if(!scope.key){scope.key=attrs.supWidgetSlot;}
scope.options={allow_fields:angular.str2opts(scope.allowFields)};element.on("mousedown",function(e){station.edit_slot(scope.key,scope.options);return e.stopPropagation();});loaded={label:attrs.label||attrs.name,key:scope.key};return station.$emit('widget.slot.loaded',loaded,scope);}};}]);}).call(this);